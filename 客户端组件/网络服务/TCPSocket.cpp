...
//代理服务器
BOOL __cdecl CTCPSocket::SetProxyServerInfo(BYTE cbProxyType, tagProxyServer ProxyServerInfo)
{
...
void * __cdecl CTCPSocket::GetTCPSocketSink(const IID & Guid, DWORD dwQueryVer)
{
	...
	return m_pITCPSocketSink;//m_pITCPSocketSink->QueryInterface(Guid,dwQueryVer);
}
...
DWORD __cdecl CTCPSocket::Connect(DWORD dwServerIP, WORD wPort)
{
	...
	if (m_hSocket!=INVALID_SOCKET) 
		throw TEXT("连接 SOCKET 句柄已经存在");
	if (m_cbSocketStatus!=SOCKET_STATUS_IDLE) 
		throw TEXT("连接状态不是等待连接状态");
	if (dwServerIP==INADDR_NONE) 
		throw TEXT("目标服务器地址格式不正确，请检查后再次尝试！");
	...
	try
	{
		...
		int iErrorCode=0;
		WSASetLastError(0);
		if (/*m_ProxyServerType!=enProxyServerType::ProxyType_None*/false)
		{
			throw TEXT("未支持代理服务器");
		}
		else
...
void __cdecl CTCPSocket::CloseSocket(BYTE cbShutReason)
{
	...
	if ((bClose==true /*&& cbShutReason==SHUT_REASON_NORMAL*/)&&(m_pITCPSocketSink!=NULL))
	{
		ASSERT(m_pITCPSocketSink!=NULL);
...
LRESULT CTCPSocket::OnSocketNotifyConnect(WPARAM wParam, LPARAM lParam)
{
	...
	else if((nErrorCode==WSAETIMEDOUT)||(nErrorCode==WSAECONNREFUSED)||(nErrorCode==WSAEADDRNOTAVAIL))
	{
		m_cbSocketStatus = SOCKET_STATUS_IDLE;
	}
	//else if ((nErrorCode==WSAETIMEDOUT)||(nErrorCode==WSAECONNREFUSED)||(nErrorCode==WSAEADDRNOTAVAIL))
