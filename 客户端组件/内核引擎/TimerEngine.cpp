...
bool /*__cdecl*/ CTimerEngine::ConcludeService()
{
	...
	m_AttemperEvent.SetQueueServiceSink(NULL);
	//设置变量
	m_TimerItemFree.Append(m_TimerItemActive);
	m_TimerItemActive.RemoveAll();
...
{
	if (m_AttemperEvent.SetQueueServiceSink(pIUnknownEx)==false)
	{
		return false;
	}
	return true;
