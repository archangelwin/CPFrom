...
bool __cdecl CDataBase::GetFieldValue(LPCTSTR lpFieldName, __int64 & llValue)
{
	try
	{
		...
		case VT_DECIMAL:
			{
				llValue = ((vtFld.decVal.sign == 128) ? -1 : 1) * vtFld.llVal;
				break;
			}
		default:
			llValue = vtFld.iVal;
...
void __cdecl CDataBase::GetParameter(LPCTSTR pszParamName, CDBVarValue & DBVarValue)
{
	...
	try
	{
		...
		if (DBVarValue.vt == VT_BSTR )
		{
			CString strTemp  = CW2CT(DBVarValue.bstrVal);
			strTemp.Replace(_T(" "),_T(""));
			DBVarValue.SetString((_bstr_t)strTemp);
		}
	}
	catch (CComError & ComError)
	{
...
{
	IDataBaseEngineSink * pIDataBaseEngineSink[10];
	for (int i=0; i < wSinkCount; ++ i)
	{
		pIDataBaseEngineSink[i] = QUERY_OBJECT_PTR_INTERFACE(pIUnknownEx[i], IDataBaseEngineSink);
	}
	m_DataBasePool.InitiDataBasePool(pIDataBaseEngineSink, wSinkCount);
	return true;
}
//启动服务
...
// 	}
	...
	if (!m_DataBasePool.DataBaseEngineStart(pIUnknownEx))
	{
...
bool  CDataBaseEngine::ConcludeService()
{
	...
	IUnknownEx * pIUnknownEx = QUERY_ME_INTERFACE(IUnknownEx);
	if (!m_DataBasePool.DataBaseEngineStop(pIUnknownEx))
	{
	...
	m_QueueServiceEvent.SetQueueServiceSink(NULL);
	m_DataBasePool.CloseDataBasePool();
	return true;
}
...
void  CDataBaseEngine::OnQueueServiceSink(WORD wIdentifier, void * pBuffer, WORD wDataSize)
{
	...
	switch (wIdentifier)
	{
		case EVENT_DATABASE:
		{
			...
			DATABASEPOOL IDataBaseEnginePool;
			if(m_DataBasePool.AllocDataBaseConnections(&IDataBaseEnginePool))
			{
				IDataBaseEnginePool.pDataBaseEngineSink->OnDataBaseEngineRequest(pDataBaseEvent->wRequestID, pDataBaseEvent->dwContextID, pDataBaseEvent + 1, wHandleBuffer);
				m_DataBasePool.RelexDataBaseConnections(&IDataBaseEnginePool);
			}
