...
BOOL CInformation::OnInitDialog()
{
	...
	if (!m_bShowTip)
	{
		nXButton=(nWindowAreaWidth-m_nBtCount*SMB_BUTTON_WIDTH-(m_nBtCount+1)*SMB_BUTTON_SPACE)/2+SMB_BUTTON_SPACE;
	}
	else
	{
		nXButton=(nWindowAreaWidth-m_nBtCount*SMB_BUTTON_WIDTH-(m_nBtCount+2)*SMB_BUTTON_SPACE)/2+SMB_BUTTON_SPACE;
	}

	if (m_bShowTip)
	{
		m_cbShowTip.SetWindowPos(NULL,nXButton,nYButton,0,0,SWP_NOSIZE|SWP_NOZORDER|SWP_SHOWWINDOW);
		nXButton+=SMB_BUTTON_SPACE+SMB_BUTTON_WIDTH;

		m_cbShowTip.SetTextColor(RGB(193,211,222),RGB(0,0,0),RGB(255,0,0));
		m_cbShowTip.SetTextColor(RGB(0,0,255),RGB(0,255,0),RGB(255,0,0));
		m_cbShowTip.SetButtonStyle(m_cbShowTip.GetButtonStyle()|BS_DEFPUSHBUTTON);
	}
...
//显示消息
INT CInformation::ShowMessageBox(LPCTSTR pszString, UINT uType, UINT nElapse,UINT nCustom,bool bShowTip)
{
	m_nCustomType = nCustom;
	m_bShowTip = bShowTip;
	return ShowMessageBox(TEXT("系统提示"),pszString,uType,nElapse,m_nCustomType,bShowTip);
}

//显示消息
INT CInformation::ShowMessageBox(LPCTSTR pszCaption, LPCTSTR pszString, UINT uType, UINT nElapse,UINT nCustom,bool bShowTip)
{
	//设置变量
	m_bShowTip = bShowTip;
	m_nCustomType = nCustom;
...
VOID CInformation::CreateBoxButton()
{
	...
	{
		m_nBtCount=2;
		uButtonID[0]=IDCANCEL;
		uButtonID[1]=IDOK;
		pszString[0]=TEXT("确定");
		pszString[1]=TEXT("立刻充值");	
	}
	else if (m_nCustomType == 2)
	{
		m_nBtCount=2;
		uButtonID[0]=IDOK;
		uButtonID[1]=IDCANCEL;
		pszString[0]=TEXT("继续进入");
		pszString[1]=TEXT("返回原游戏");	
	}
	else if(m_nCustomType == 3)
	{
		m_nBtCount=2;
		uButtonID[0]=IDOK;
		uButtonID[1]=IDCANCEL;
	...
	CRect rcShowTip(0,0,70,16);
	if (m_bShowTip)
	{
		m_cbShowTip.Create("不再提示",WS_CHILD | BS_AUTOCHECKBOX | BS_FLAT,rcShowTip,this,IDCONTINUE);
	}	
...
//显示消息
extern "C" SHARE_CONTROL_CLASS INT ShowInformation(LPCTSTR pszString, UINT uType, UINT nElapse,UINT nCustom,bool bShowTip)
{
	CInformation Information;
	return Information.ShowMessageBox(pszString,uType,nElapse,nCustom,bShowTip);
}

//显示消息
extern "C" SHARE_CONTROL_CLASS INT ShowInformationEx(LPCTSTR pszCaption, LPCTSTR pszString, UINT uType, UINT nElapse,UINT nCustom,bool bShowTip)
{
	CInformation Information;
	return Information.ShowMessageBox(pszCaption,pszString,uType,nElapse,nCustom,bShowTip);
}
