...
interface IMissionSocketSink
{
	...
	////重连事件
	//virtual void OnEventMissionReConnect(int nErrorCode)=NULL;
	//关闭事件
...
public:
	//中断连接
	bool PerformClose(BYTE cbShutReason = SHUT_REASON_NORMAL);
	//发起连接
...
protected:
	...
	//重连事件
	//virtual void OnEventMissionReConnect(int nErrorCode);
	//关闭事件
