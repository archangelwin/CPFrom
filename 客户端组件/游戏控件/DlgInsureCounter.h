...
#define INSURE_TRANSFERRECORD       2                                   //转账记录
...
public:
	TCHAR							m_szSaveInsurePass[LEN_PASSWORD];		//银行密码
...
private:
	bool							m_bInit;							//是否已初始化
	int								m_nBeginTop;						//下面开始
	int								m_nHSpace;							//子项上下间隔
	int								m_nLSpace;							//左边开始位置
	CFont							m_BankFont;							//银行信息字体
	CFont							m_BankFontInfo;						//银行子项字体
	CRect							m_BankRect[4];						//银行子项区域
	int								m_nSelIndex;						//当前选中项
	CBitImage						m_ImageSelBank;					
	//子项窗口
protected:
	CDlgInsureItem					m_DlgInsureItem;					//银行子项
	CDlgInsureTeller				m_DlgInsureTeller;					//取款
	CDlgInsureSave					m_DlgInsureSave;					//存款
	CDlgInsureTransfer				m_DlgInsureTransfer;				//转账子项
	CDlgBrowserItem                 m_DlgBrowserItem;                   //转账记录
