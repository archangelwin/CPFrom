...
VOID CUserItemElement::ConstructNameCard(IClientUserItem * pIClientUserItem, IGameLevelParser * pIGameLevelParser, CImage&ImageNameCard)
{
	...
	//位置定义
	CRect rcNickName(2,5,ImageSize.cx-5,5+12);
	CRect rcGameLevel(2,25+5,ImageSize.cx-5,25+12+5);
	CRect rcExperience(2,45,ImageSize.cx-5,45+12);
	CRect rcUserGameID(2,65,ImageSize.cx-5,65+12);
	CRect rcUserUnderWrite(2,25,ImageSize.cx-5,85+12);
...
WORD CUserItemElement::GetExperienceLevel(DWORD dwExperience)
{
	if (dwExperience>0L)
	{
		...
		//等级计算
		WORD wUserLevel=1;
		for (wUserLevel;wUserLevel<60L;wUserLevel++)
		{
...
bool CUserItemElement::EfficacyAccounts(LPCTSTR pszAccounts, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if (nCharLength<LEN_LESS_ACCOUNTS)
	{
		...
		{
			_sntprintf(szDescribe,wMaxCount,TEXT("游戏帐号必须大于 %ld 位以上！"),LEN_LESS_ACCOUNTS);
			return false;
		}
	}

	if ((pszAccounts[0]>=TEXT('0'))&&(pszAccounts[0]<=TEXT('9')))
	{
		_sntprintf(szDescribe,wMaxCount,TEXT("游戏帐号首字符不能为数字！"),LEN_LESS_ACCOUNTS);
		return false;
	}

	if (CWHService::BAllNumber(pszAccounts))
	{
		_sntprintf(szDescribe,wMaxCount,TEXT("游戏帐号不能全为数字！"),LEN_LESS_ACCOUNTS);
		return false;
	}
...
bool CUserItemElement::EfficacyNickName(LPCTSTR pszNickName, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if (nCharLength<LEN_LESS_NICKNAME)
	{
		...
		{
			_sntprintf(szDescribe,wMaxCount,TEXT("游戏昵称必须大于 %ld 位以上！"),LEN_LESS_NICKNAME);
			return false;
		}
	}
...
//效验真实名字
bool CUserItemElement::EfficacyRealName(LPCTSTR pszRealName, TCHAR szDescribe[], WORD wMaxCount)
{
	//变量定义
	INT nCharLength=lstrlen(pszRealName);
	INT nDataLength=nCharLength*sizeof(TCHAR);

	//长度判断
	if (nCharLength<LEN_LESS_REALNAME)
	{
		//变量定义
		WORD nChinessCount=0;
		...
		for (INT i=0;i<nCharLength;i++)
		{
			if (pszRealName[i]>=0x80)
			{
				nChinessCount++;
			}
		}
		//长度判断
		if ((nChinessCount+nCharLength)<LEN_LESS_REALNAME)
		{
			_sntprintf(szDescribe,wMaxCount,TEXT("真实名字必须大于 %ld 位以上！"),LEN_LESS_REALNAME);
			return false;
		}
	}
	else
	{
		...
		for (INT i=0;i<nCharLength;i++)
		{
			if ((pszRealName[i]>=TEXT('0'))&&(pszRealName[i]<=TEXT('9')))
			{
				bHaveNumber = true;
				break;
			}
		...
		if (bHaveNumber)
		{
			_sntprintf(szDescribe,wMaxCount,TEXT("请输入正确的真实姓名！"),"");
			return false;
		}
...
bool CUserItemElement::EfficacyPassword(LPCTSTR pszPassword, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	{
		_sntprintf(szDescribe,wMaxCount,TEXT("密码必须大于 %ld 位以上！"),LEN_LESS_PASSWORD);
		return false;
	}
	else
	{
		CString strString = pszPassword;
		CString sFilterSrc[4] = {"&","<",">","%"};
		for(int i(0) ; i < 4 ; i ++)
		{	
			if (strString.Find(sFilterSrc[i])!=-1)
			{
				_sntprintf(szDescribe,wMaxCount,TEXT("密码不能含有特殊字符！"),LEN_LESS_PASSWORD);
				return false;
			}
		}
	}
	CString strPwd = pszPassword;
	strPwd.TrimLeft();
	strPwd.TrimRight();
	if(strPwd.IsEmpty())
	{
	...
	strPwd.TrimLeft("0123456789");
	// 如果去掉数字后，字串为空，说明字串中全部是数字
	// 如果不允许第一个字符为0，前面再加一个判断if(str.GetAt(0) != '0') return FALSE;
	if(strPwd.IsEmpty())	
	{
		_sntprintf(szDescribe,wMaxCount,TEXT("密码不能全是数字！"),LEN_LESS_PASSWORD);
		return false;
	}
...
bool CUserItemElement::EfficacyPassPortID(LPCTSTR pszPassPortID, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if(nCharLength==0L) 
	{
		lstrcpyn(szDescribe,TEXT("身份证号码必须为 18 位数字！"),wMaxCount);
		return false;
	}
...
//	if ((bNumber==false)||((nCharLength!=15L)&&(nCharLength!=18L)))
	...
	{
		lstrcpyn(szDescribe,TEXT("您输入的身份证号码不正确！"),wMaxCount);
		return false;
	}
