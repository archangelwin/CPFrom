#pragma once
#include "ButtonEx.h"
#include "..\AdminControl\UserServerInfo.h"
#include <map>
//------------------------------------------------------------------------
...
#define WARNING_LV1   1000
#define WARNING_LV2   2000
#define WARNING_LV3   3000

typedef struct tagButtonKey
{
	int m_ButtonRow;
	int m_ButtonCol;

	bool operator <(const tagButtonKey& other) const // 注意是const函数！！
	{
		if (m_ButtonCol != other.m_ButtonCol) // 类型按升序排序
		{
			return (m_ButtonCol < other.m_ButtonCol);
		}
		else // 如果类型相同，按比例尺升序排序
		{
			return (m_ButtonRow < other.m_ButtonRow);
		}
	}

} ButtonKey;

typedef	map<ButtonKey,CButtonEx*> button_map;
...
public:
	...
	//
public:
	void createItemButton(LPCTSTR title, int nItem, int nSubItem, HWND hMain );
	void updateListCtrlButtonPos();
	void enableButton( BOOL bFlag, int iItem );
	void release();
	void deleteItemEx( int nItem,int nCol );
...
public:
	...
	virtual afx_msg BOOL OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult);
