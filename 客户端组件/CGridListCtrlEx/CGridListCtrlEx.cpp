...
#include <afxole.h>		// 
#include "..\AdminControl\DlgUserInfo.h"
...
BEGIN_MESSAGE_MAP(CGridListCtrlEx, CListCtrl)
	...
	ON_NOTIFY_REFLECT_EX(LVN_BEGINDRAG, OnBeginDrag)		// Begin Drag Dropb
		//ON_NOTIFY_REFLECT(LVN_ENDSCROLL, &CGridListCtrlEx::OnLvnEndScroll)
...
#if __STDC_WANT_SECURE_LIB__
		_mbstowcsz(/*NULL, */pTTTW->szText, /*static_cast<LPCTSTR>*/(tooltip), sizeof(pTTTW->szText)/sizeof(WCHAR));
#else
...
// 	pContextMenu->LoadMenu(IDR_MENU2);
...
BOOL CGridListCtrlEx::OnItemClick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (GetStyle() & LVS_OWNERDATA && GetExtendedStyle() & LVS_EX_CHECKBOXES)
	{
		...
		if (nRow!=-1)
		{
			...
			if (pGameUserInfoItem != NULL)
			{
				if (pGameUserInfoItem->m_tagUMUserScoreSet.nVipType == 1)
				{
					CMenu menu, *pPopup;  
...
void CGridListCtrlEx::createItemButton(LPCTSTR title, int nItem, int nSubItem, HWND hMain )
{
	CRect rect;
	int offset = 0;
...
// 		rect.right = rcClient.right;
	...
	m_uID++;
	pButton->Create(title,dwStyle, rect, this, m_uID);
	ButtonKey key;
	key.m_ButtonCol = nSubItem;
	key.m_ButtonRow = nItem;
	m_mButton.insert( make_pair(key , pButton ) );
...
void CGridListCtrlEx::OnLvnEndScroll(NMHDR *pNMHDR, LRESULT *pResult)
{
...
{
	// TODO: 偙偙偵儊僢僙乕僕 僴儞僪儔 僐乕僪傪捛壛偡傞偐丄婛掕偺張棟傪屇傃弌偟傑偡丅

	CListCtrl::OnDrawItem(nIDCtl, lpDrawItemStruct);
}

void CGridListCtrlEx::updateListCtrlButtonPos()
{
// 	int iTopIndex = GetTopIndex();
// 	int nItem = iTopIndex;
// 	ButtonKey key;
...
void CGridListCtrlEx::enableButton( BOOL bFlag, int iItem )
{
// 	button_map::iterator iter;
// #ifdef USE_TOPINDEX_BUTTON
// 	int iTopIndex = GetTopIndex();
...
void CGridListCtrlEx::deleteItemEx( int nItem,int nCol )
{
	...
	ButtonKey key;
	key.m_ButtonRow = nItem;
	key.m_ButtonCol = nCol;
#ifdef USE_TOPINDEX_BUTTON
	//add-----------------------------------
	iter = m_mButton.find( key );
	iterNext = iter;
	iterNext++;
...
BOOL CGridListCtrlEx::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
...
// 		updateListCtrlButtonPos();
// 		// here add your codes...
// 		if (pHDN->iItem == 19 || pHDN->iItem == 18)
// 		{
// 			*pResult = TRUE;
// 			return TRUE;
// 		}
...
void CGridListCtrlEx::SetItemColor(int col,int row,COLORREF color) //设置格子颜色
{
...
void CGridListCtrlEx::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
bool CGridListCtrlEx::FindColColor(int col,COLORREF &color) //查找列颜色
{
	int flag = 0;
...
bool CGridListCtrlEx::FindItemColor(int col,int row,COLORREF &color) //查找颜色
{
	int flag = 0;
...
void CGridListCtrlEx::SetColColor(int col,COLORREF color) //设置列颜色
{
	stColor *pColor  = new stColor;
	pColor->nCol = col;
	pColor->rgb = color;
...
void CGridListCtrlEx::SetRowHeigt(int nHeight) //高置行高
{
...
void CGridListCtrlEx::SetHeaderFontHW(int nHeight,int nWith) //设置头部字体宽和高
{
// 	m_Header.m_fontHeight = nHeight;
// 	m_Header.m_fontWith = nWith;
}
void CGridListCtrlEx::SetHeaderTextColor(COLORREF color) //设置头部字体颜色
{
	//m_Header.m_color = color;
}
BOOL CGridListCtrlEx::SetTextColor(COLORREF cr)  //设置字体颜色
{
	m_color = cr;
	return TRUE;
}
void CGridListCtrlEx::SetFontHW(int nHeight,int nWith) //设置字体高和宽
{
	m_fontHeight = nHeight;
	m_fontWith = nWith;
}
void CGridListCtrlEx::SetColTextColor(int col,COLORREF color)
{
	stColor *pColor = new stColor;
	pColor->nCol = col;
	pColor->rgb = color;
...
bool CGridListCtrlEx::FindColTextColor(int col,COLORREF &color)
{
	int flag = 0;
...
bool CGridListCtrlEx::FindItemTextColor(int col,int row,COLORREF &color)
{
	int flag = 0;
...
void CGridListCtrlEx::SetItemTextColor(int col,int row,COLORREF color)
{
	stColor *pColor = new stColor;
	pColor->nCol = col;
	pColor->nRow = row;
