...
{
	CString strLeft = pszLeftValue, strRight = pszRightValue;
	strLeft.Replace(",", "");
	strRight.Replace(",", "");
	if(_tcscmp( pszLeftValue, "0" )==0 || _tcscmp( pszRightValue, "0" )==0 || atoi(strLeft) != 0 || atoi(strRight) != 0)
	//if (m_SortFormatNumber)
	{
		if (bAscending)
			return atoi(strLeft) > atoi(strRight);
		else
			return atoi(strLeft) < atoi(strRight);

		TCHAR szBufferLeft[64]=TEXT("");
		TCHAR szBufferRight[64]=TEXT("");
		WORD wIndexLeft=0;
		WORD wIndexRight=0;
		SCORE lScoreLeft=0L;
		SCORE lScoreRight=0L;
		lstrcpyn(szBufferLeft,pszLeftValue,sizeof(szBufferLeft));
		lstrcpyn(szBufferRight,pszRightValue,sizeof(szBufferRight));
		//构造数值
		while (szBufferLeft[wIndexLeft]!=0)
		{
			//变量定义
			WORD wCurrentIndex=wIndexLeft++;

			//构造数值
			if ((szBufferLeft[wCurrentIndex]>=TEXT('0'))&&(szBufferLeft[wCurrentIndex]<=TEXT('9')))
			{
				lScoreLeft=lScoreLeft*10L+(szBufferLeft[wCurrentIndex]-TEXT('0'));
			}
		}

		while (szBufferRight[wIndexRight]!=0)
		{
			//变量定义
			WORD wCurrentIndex=wIndexRight++;

			//构造数值
			if ((szBufferRight[wCurrentIndex]>=TEXT('0'))&&(szBufferRight[wCurrentIndex]<=TEXT('9')))
			{
				lScoreRight=lScoreRight*10L+(szBufferRight[wCurrentIndex]-TEXT('0'));
			}
		}
...
// 		int nRightValue = _ttoi(pszRightValue);
		if (bAscending)
			return lScoreLeft - lScoreRight;
		else
			return lScoreRight - lScoreLeft;
	}
	else
	{
		if (bAscending)
...
{
	const CString testText = _T("yjp土");
