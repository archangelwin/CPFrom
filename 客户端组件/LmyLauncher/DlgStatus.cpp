...
BOOL CDlgStatus::OnInitDialog()
{
	...
	//设置标题
	SetWindowText("龙马游更新程序");

	//设置控件
	SetTimer(IDT_STATUS,TIMER_STATUS,NULL);

	//加载资源
	CPngImageSB ImageStatus;
	ImageStatus.LoadCImage(AfxGetInstanceHandle(),TEXT("STATUS_IMAGE"));
	...
	//设置大小
	CSize SizeWindow(ImageStatus.GetCWidth(),ImageStatus.GetCHeight());
...
BOOL CDlgStatus::OnEraseBkgnd(CDC * pDC)
{
	...
	CPngImageSB ImageStatus;
	ImageStatus.LoadCImage(AfxGetInstanceHandle(),TEXT("STATUS_IMAGE"));
	ImageStatus.DrawCImage(pBufferDC,0,0,rcClient.Width(),rcClient.Height(),0,0);

	//加载资源
	CPngImageSB ImageStatusLine;
	CPngImageSB ImageStatusPoint;
	ImageStatusLine.LoadCImage(AfxGetInstanceHandle(),TEXT("STATUS_LINE"));
	ImageStatusPoint.LoadCImage(AfxGetInstanceHandle(),TEXT("STATUS_POINT"));

	//获取大小
	CSize SizeStatusLine;
	CSize SizeStatusPoint;
	SizeStatusLine.SetSize(ImageStatusLine.GetCWidth(),ImageStatusLine.GetCHeight()/2);
	SizeStatusPoint.SetSize(ImageStatusPoint.GetCWidth(),ImageStatusPoint.GetCHeight());

	//绘画进度
	ImageStatusLine.DrawCImage(pBufferDC,105,45,SizeStatusLine.cx,SizeStatusLine.cy,0,0);
	ImageStatusLine.DrawCImage(pBufferDC,105,45,SizeStatusLine.cx*m_uImageIndex/100L,SizeStatusLine.cy,0,SizeStatusLine.cy);

	//绘画进度
	//INT nXPoint=SizeStatusLine.cx*m_uImageIndex/100L;
	//ImageStatusPoint.DrawCImage(pBufferDC,105+nXPoint-SizeStatusPoint.cx/2,45+(SizeStatusLine.cy-SizeStatusPoint.cy)/2);
