// DES2.cpp: implementation of the DES2 class.
//
//////////////////////////////////////////////////////////////////////
...
void DES2::InitializeKey(CString StrKey,unsigned int keyN)
{
	char SourceKey[8];	
	memcpy(SourceKey,StrKey.GetBuffer(StrKey.GetLength()),StrKey.GetLength() < 8 ? StrKey.GetLength() : 8);
	//convert 8 char-bytes key to 64 binary-bits
	char sz_64key[64] = {0};
	Bytes2Bits(SourceKey,sz_64key,64);
	//PC 1
	char sz_56key[56] = {0};
	for(int k=0;k<56;k++)
	{
...
void DES2::ConvertCiphertext2OtherFormat(int iBitsLen,char *szCipherInBytes)
{
	memset(hexCiphertextAnyLength,0,16384);
	memset(bitsCiphertextAnyLength,0,32768);
...
void DES2::EncryptAnyLength(char* _srcBytes,unsigned int _bytesLength,unsigned int keyN)
{
	...
	ConvertCiphertext2OtherFormat(_bytesLength % 8 == 0 ? _bytesLength << 3 : ((_bytesLength>>3) + 1) << 6,szFCiphertextAnyLength);
}

void DES2::EncryptAnyLength(CString strEncry,unsigned int keyN)
{
	char _srcBytes[8192];	
	memset(_srcBytes,0,8192);
	memcpy(_srcBytes,strEncry.GetBuffer(strlen(strEncry)),strEncry.GetLength());
	unsigned int _bytesLength = strlen(strEncry);
	if(_bytesLength == 8)
	{
		EncryptData(_srcBytes,keyN);
		memcpy(szFCiphertextAnyLength,szCiphertextInBytes,8);
