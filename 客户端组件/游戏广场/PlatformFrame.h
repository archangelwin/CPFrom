...
public:
	...
	CSkinButton						m_btCertificate;					//证件

	//控制按钮
public:
	//CSkinButton						m_btControl;						//控制按钮
	CSkinButton						m_btLockList;						//锁定按钮
	CSkinButton						m_btGameList;						//列表按钮
	CSkinButton						m_btGameMatch;						//比赛按钮
...
protected:
	...
	CDlgInsurePlaza *               m_pInsurePlaza;                     //保险柜
...
protected:
	...
	CString							m_sRemInsurePwd;		            //保存的保险柜密码
...
public:
	...
	//进入房间
	CServerViewItem * EntranceServerItem(CGameServerItem * pGameServerItem,bool bOpenInsureDlg = false);
...
protected:
	...
	LRESULT OnSafeBind(WPARAM wParam, LPARAM lParam);
...
public:
	...
	bool RegisterDll();
	bool GetPlayingGameServer(int &nGameServerID); //获取第一个正在玩的游戏房间id，无则返回-1
	void ExitGameServer(int nGameServerID);
	void ActiveGameServe(int nGameServerID);
	void PopSysMessage();
	void ClosePazza(int nType); //1,关闭客户端;2,切换账号
	CString GetRemInsurePwd(){return m_sRemInsurePwd;};
	void SetRemInsurePwd(CString sNewInsurePwd);
	void SaveInsurePwd(int nType,int nMinute,LPCTSTR pszInsurePwd);
	void InitData();
