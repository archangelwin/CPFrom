...
BOOL CCollocateProxy::OnInitDialog()
{
	...
	//变量定义
	BYTE cbProxyType[]={enProxyServerType::ProxyType_None,enProxyServerType::ProxyType_Http,enProxyServerType::ProxyType_Socks4,enProxyServerType::ProxyType_Socks5};
...
bool CCollocateProxy::UpdateControlStatus()
{
	...
	//设置选择
	INT i=0;
	for (i;i<m_cmProxyType.GetCount();i++)
	{
...
VOID CCollocateProxy::OnBnClickedProxyTest()
{
	...
	//效验代理
	if (cbProxyType!=enProxyServerType::ProxyType_None)
	{
		//代理地址
	...
	//创建组件
	CTCPSocketHelper WHNetworkModule;
	...
	//代理测试
	switch (WHNetworkModule->SetProxyServerInfo(cbProxyType,ProxyServer))
	{
