...
VOID CDlgServiceIndividual::OnOK()
{
	...
	{
		lstrcpyn(m_szPassword,pGlobalUserData->szPassword,CountArray(m_szPassword));
	}
	else
	{
...
VOID CDlgServiceIndividual::LoadAccountsInfo()
{
	...
	if(pGlobalUserInfo->GetUserAccountType() >0)
	{
		...
		GetDlgItem(IDC_NOTE)->ShowWindow(SW_HIDE);
		GetDlgItem(IDC_STATIC_INPWD)->ShowWindow(SW_HIDE);
	}
	...
	else
	{
		DWORD faceID;
		SCORE nUserScore;
		nUserScore = pGlobalUserData->lUserScore + pGlobalUserData->lUserInsure;
		if (nUserScore==0)
			faceID=0;
		...
		if (pGlobalUserData->cbGender == 0)
		{
			faceID += 13;
		}

//		m_FaceItemView.SetSystemFace(pGlobalUserData->wFaceID);
		m_FaceItemView.SetSystemFace(faceID);
	}
