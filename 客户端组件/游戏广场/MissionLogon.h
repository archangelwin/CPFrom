...
protected:
	...
	int                             m_LogonSocketType;                  //登录socket状态:(0,登录;1退出大厅;2切换账号)
	bool                            m_bHaveLoginIn;                     //是否已经登录
...
public:
	...
	//状态接口
public:
	void SetLoginSocketType(int SocketType); //socket状态:(0,登录;1退出大厅;2切换账号)
	void SetHaveLogonIn(bool bHaveLogonIn);
	//取消连接
...
public:
	...
	//显示注册
	VOID ShowRegister();
	bool BHaveLogonIn(){return m_bHaveLoginIn;};
	//发送信息
	bool SendLogonPacket(BYTE cbValidateFlags,int nValidateType = 0,CString sCode = "");
...
protected:
	...
	bool OnSocketSubClosePazza(VOID * pData, WORD wDataSize);
