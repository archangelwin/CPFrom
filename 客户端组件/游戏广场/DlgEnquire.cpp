...
BOOL CDlgEnquire::OnInitDialog()
{
	...
	m_btClosePlaza.SetButtonImage(IDB_ENQUIRE_PLAZA,hInstance,false,false);
	m_btCloseServer.SetButtonImage(IDB_ENQUIRE_SERVER,hInstance,false,false);
	m_btSwitchAccounts.SetButtonImage(IDB_ENQUIRE_SWITCH,hInstance,false,false);

	//关闭按钮
	CRect rcCancel;
	m_btCancel.GetWindowRect(&rcCancel);
	m_btCancel.SetWindowPos(NULL,SizeWindow.cx-rcCancel.Width()-15,15-10,0,0,SWP_NOZORDER|SWP_NOSIZE|SWP_NOREDRAW);
	...
	_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/Ads/ClosePlaza.aspx"),pGlobalServer->GetPlatformServer(3));

	//设置广告
	m_PlatformPublicize.Navigate(szBillUrl);
	m_PlatformPublicize.SetWindowPos(NULL,7,58-22,SizeWindow.cx-14,80+25,SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOACTIVATE);
