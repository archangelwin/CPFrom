...
BOOL CGamePlazaApp::InitInstance()
{
	...
	//全局配置l
	m_GlobalUnits.InitGlobalUnits();
	CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
	int nMulti = pGlobalServer->GetPlatInfo(TEXT("PlatInfo"),TEXT("Multi"),0);
...
// 			return FALSE;
	...
	//设置表名
	SetRegistryKey(pGlobalServer->GetPlatformProduct(3));
	...
	pPlatformFrame->Create(pGlobalServer->GetPlatformProduct(2),pGlobalServer->GetPlatformProduct(1),WS_CLIPCHILDREN|WS_CLIPSIBLINGS|WS_SYSMENU,CRect(0,0,0,0));
	...
	CD3DSound::GetInstance()->CreateD3DSound(m_pMainWnd->GetSafeHwnd());
	return TRUE;
}
