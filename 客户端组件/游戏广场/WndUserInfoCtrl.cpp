...
//控制按钮
#define IDC_USER_INFO_1				100									//元宝兑换
#define IDC_USER_INFO_2				101									//安全绑定
#define IDC_USER_INFO_3				102									//魅力兑换
#define IDC_USER_INFO_4			    103									//救济金
#define IDC_USER_INFO_SET			104									//用户按钮
#define IDC_USER_INFO_EDIT			105									//编辑控件
#define IDC_USER_INFO_COPY          106                                 //用户信息复制
...
CWndUserInfoCtrl::CWndUserInfoCtrl()
{
	...
	//设置区域
	m_rcFaceArea.SetRect(13,24,69,80);
	...
	case IDC_USER_INFO_1:		//元宝兑换
		{
			...
			//构造地址
			//CString sUrl;
			//sUrl.Format("Ingot.aspx?t=%d",time(NULL));
			//TCHAR szLogonLink[256]=TEXT("");
			//_sntprintf(szLogonLink,CountArray(szLogonLink),TEXT(sUrl));

			////打开页面
			//CDlgBrowser DlgBrowser;
			//DlgBrowser.CreateBrowserControl(TEXT("元宝兑换"),szLogonLink,CSize(730,450));
			CDlgService DlgService;
			DlgService.SetExpectationID(2);
			DlgService.DoModal();
	...
	case IDC_USER_INFO_3:		//魅力兑换
		{
			//构造地址
			CString sUrl;
			sUrl.Format("LoveLiness.aspx?t=%d",time(NULL));
			TCHAR szLinkUrl[256]=TEXT("");
			_sntprintf(szLinkUrl,CountArray(szLinkUrl),TEXT(sUrl));

			//打开页面
			CDlgBrowser DlgBrowser;
			DlgBrowser.CreateBrowserControl(TEXT("魅力兑换"),szLinkUrl,CSize(730,450));
			return TRUE;
		}
	case IDC_USER_INFO_4:  //绑定手机
		{
			//显示窗口
// 			CAlmsDlg DlgAlms;
// 			DlgAlms.DoModal();
...
// 			_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/MachineBind/UserBindMachine2.aspx"),pGlobalServer->GetPlatformServer(3));
			...
			DlgMobileBind.SetWindowSize(490,210,490,/*210-107+100*/170+50);
	...
	case IDC_USER_INFO_COPY:
		{
			...
			CString szUserInfoCopy;
			szUserInfoCopy.Format("%s %d",pGlobalUserData->szNickName,pGlobalUserData->dwGameID);
			...
			{   
				...
				char *buffer;   
				EmptyClipboard();   
				clipbuffer=GlobalAlloc(GMEM_DDESHARE,szUserInfoCopy.GetLength()+1);   
				buffer=(char*)GlobalLock(clipbuffer);   
				strcpy(buffer,LPCSTR(szUserInfoCopy));   
			...
			Information.ShowMessageBox(TEXT("已经复制用户昵称和游戏ID"),MB_ICONINFORMATION,10);
...
INT CWndUserInfoCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//加载资源
	CBitImage ImageBK;
	ImageBK.LoadFromResource(AfxGetInstanceHandle(),IDB_USER_INFO_BK);
	...
	m_btUserInfo4.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_USER_INFO_4);  
	...
	//m_btUserInfo3.ShowWindow(SW_HIDE);

	//设置按钮
	HINSTANCE hInstance=AfxGetInstanceHandle();
	m_btUserInfo1.SetButtonImage(IDB_BK_USER_INFO_1,NULL,hInstance,true,false);
	m_btUserInfo2.SetButtonImage(IDB_BK_USER_INFO_2,NULL,hInstance,true,false);
	//m_btUserInfo2.SetButtonImage(IDB_USER_INFO_SAFA,hInstance,true,false);
	m_btUserInfo3.SetButtonImage(IDB_BK_USER_MEILI,hInstance,true,false);
	m_btUserInfo4.SetButtonImage(IDB_BK_USER_INFO_4,NULL,hInstance,true,false);
	...
	SetWindowPos(NULL,0,0,rcUserInfo.Width(),ImageBK.GetHeight(),SWP_NOMOVE|SWP_NOZORDER);

	//提示控件
	m_ToolTipCtrl.Create(this);
	m_ToolTipCtrl.Activate(TRUE);
	m_ToolTipCtrl.AddTool(&m_btUserInfo1,TEXT("个人设置"));
	m_ToolTipCtrl.AddTool(&m_btUserInfo2,TEXT("安全绑定"));
	m_ToolTipCtrl.AddTool(&m_btUserInfo3,TEXT("魅力兑换"));
	m_ToolTipCtrl.AddTool(&m_btUserInfo4,TEXT("绑定手机"));
	m_ToolTipCtrl.AddTool(&m_btUserCopy,TEXT("复制"));
//	m_ToolTipCtrl.AddTool(&m_btUserInfoSet,TEXT("密保设置"));
	/*m_EditUnderWrite.ShowWindow(SW_HIDE);*/
...
VOID CWndUserInfoCtrl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//按钮大小
	CRect rcButton1;
	m_btUserInfo1.GetWindowRect(&rcButton1);
	CRect rcButton2;
	m_btUserInfo2.GetWindowRect(&rcButton2);
	CRect rcButton4;
	m_btUserInfo4.GetWindowRect(&rcButton4);
	CRect rcButtonCopy;
	m_btUserCopy.GetWindowRect(&rcButtonCopy);

	//间隔定义
	UINT nSpace=4;
	UINT nStartPos=5+50+30-5;
	nSpace = (nWidth-nStartPos*2)/3;
	//调整按钮
//	DeferWindowPos(hDwp,m_btUserInfoSet,NULL,69,25,0,0,uFlags);
	DeferWindowPos(hDwp,m_btUserInfo1,NULL,nStartPos,nHeight-rcButton2.Height()-3,0,0,uFlags);
	DeferWindowPos(hDwp,m_btUserInfo2,NULL,nStartPos+rcButton2.Width(),nHeight-rcButton2.Height()-3,0,0,uFlags);
	DeferWindowPos(hDwp,m_btUserInfo4,NULL,nStartPos+rcButton2.Width()+rcButton1.Width(),nHeight-rcButton2.Height()-3,0,0,uFlags);

	DeferWindowPos(hDwp,m_btUserCopy,NULL,nStartPos+rcButton2.Width()+rcButton1.Width()+18,nHeight-rcButton2.Height()-3-60,0,0,uFlags);

	DeferWindowPos(hDwp,m_btUserInfo3,NULL,nStartPos+rcButton2.Width()+rcButton1.Width()+18,10+30,0,0,uFlags);
	//结束调整
...
BOOL CWndUserInfoCtrl::OnEraseBkgnd(CDC * pDC)
{
	...
	CBitImage ImageBK;
	ImageBK.LoadFromResource(hInstance,IDB_USER_INFO_BK);
	...
	pSkinRenderManager->RenderImage(ImageBK);

	//绘画背景
	ImageBK.BitBlt(BufferDC,0,0);
	...
	/*	//绘画底框
		...
		{
			//加载资源
			CPngImageSB ImageUnderWriteL;
			CPngImageSB ImageUnderWriteM;
			CPngImageSB ImageUnderWriteR;
...
*/
		...
		{
			theApp.m_FaceItemControlModule->DrawFaceItemFrame(pBufferDC,17,11,ITEM_FRAME_HOVER);
		}
		...
		if (pGlobalUserData->CustomFaceInfo.dwDataSize!=0)
		{
			...
			theApp.m_FaceItemControlModule->DrawFaceNormal(pBufferDC,17+1,11,pdwCustomFace);
		}
		else
		{
			theApp.m_FaceItemControlModule->DrawFaceNormal(pBufferDC,17+1,11,pGlobalUserData->wFaceID);
		}

		//绘画标题
		pBufferDC->SetTextColor(RGB(255,255,255));
		//pBufferDC->TextOut(88,16-10,TEXT("昵  称："));
		//pBufferDC->TextOut(88,36-10,TEXT("游戏ID："));
		//pBufferDC->TextOut(88,56-10,TEXT("金  币："));

		//构造奖牌
		TCHAR szUserMedal[64]=TEXT("");
		_sntprintf(szUserMedal,CountArray(szUserMedal),TEXT(" %ld"),pGlobalUserData->dwGameID);

		//输出奖牌
		CRect rcUserMedal;
		rcUserMedal.SetRect(130,36-10,rcClient.Width()-15,48+5-12);
		pBufferDC->DrawText(szUserMedal,lstrlen(szUserMedal),&rcUserMedal,DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS);

		//输出帐号
		CRect rcNickName;
		rcNickName.SetRect(137,16-10,rcClient.Width()-15,28-9);
...
// 		TCHAR szUserIngot[64]=TEXT("");
// 		_sntprintf(szUserIngot,CountArray(szUserIngot),TEXT(" %I64d"),pGlobalUserData->lUserIngot);
// 		pBufferDC->DrawText(szUserIngot,lstrlen(szUserIngot),&rcUnderWrite,DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS);

		TCHAR szUserScore[64]=TEXT("");
		_sntprintf(szUserScore,CountArray(szUserScore),TEXT(" %I64d"),pGlobalUserData->lUserScore);
		rcUnderWrite.SetRect(130,56-10,rcClient.Width()-15,68+10-18);
		pBufferDC->DrawText(szUserScore,lstrlen(szUserScore),&rcUnderWrite,DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS);
	}
	...
	case EVENT_USER_LOGON:			//登录成功
		{
			...
			CGlobalUserInfo * pGlobalUserInfo=CGlobalUserInfo::GetInstance();
			tagGlobalUserData * pGlobalUserData=pGlobalUserInfo->GetGlobalUserData();
			//pGlobalUserData->cbMemberOrder=1;
			if (pGlobalUserData->cbMemberOrder>=1)
			{
				m_btUserInfo3.ShowWindow(SW_SHOW);
			}
			else
			{
				m_btUserInfo3.ShowWindow(SW_HIDE);
			...
			m_btUserInfo4.EnableWindow(TRUE);
	...
	case EVENT_USER_LOGOUT:			//注销成功
		{
			...
			m_btUserInfo4.EnableWindow(FALSE);
			m_btUserCopy.EnableWindow(FALSE);
