...
BEGIN_MESSAGE_MAP(CDlgDownLoad, CSkinDialog)
	...
	ON_BN_CLICKED(IDC_DOWNLOAD_DX, OnBnClickedDX)
	ON_BN_CLICKED(IDC_DOWNLOAD_WT, OnBnClickedWT)
	ON_STN_CLICKED(IDC_STATIC_WEBDOWN1, OnBnClickedWebGameDownLowd)
...
bool CDownLoadSink::CloseDownLoad()
{
	...
	m_pDlgDownLoad->m_DownLogFile.Write(m_sDownLoadUrl + "  关闭下载");

	return true;
}

//执行下载
bool CDownLoadSink::PerformDownLoad(LPCTSTR pszClientName, WORD wKindID, WORD wServerID,LPCTSTR szProcessName)
{
	//效验状态
	...
	m_pDlgDownLoad->m_DownLogFile.Write("-----------------------------下载   " + (CString)szProcessName + "------------------------------------");
	//下载速度
	m_dwDownSpeed=0L;
	m_dwLastCalcSize=0L;
	...
	//设置变量
	m_sDownLoadFileName = szProcessName;
	m_wKindID=wKindID;
	...
	//_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("%s/DownLoad.aspx?KindID=%ld"),pGlobalServer->GetPlatformServer(5),wKindID);
	_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("%s/DownLoad/%s"),pGlobalServer->GetPlatformServer(5),szProcessName);
	m_sDownLoadUrl  = szDownLoadUrl;
	//发送状态
	m_pDlgDownLoad->SendMessage(WM_ITEM_STATUS_UPDATE,(WPARAM)this,0);

	//执行下载
	bool bConnectResult =false;
	bConnectResult = m_DownLoad.PerformDownLoad(szDownLoadUrl,QUERY_ME_INTERFACE(IUnknownEx));
	if (!bConnectResult)
		m_pDlgDownLoad->m_DownLogFile.Write(m_sDownLoadUrl + "  连接失败");

	return bConnectResult;
}

//执行下载
bool CDownLoadSink::PerformDownLoad(LPCTSTR pszClientName, LPCTSTR szDownloadIP,LPCTSTR szProcessName)
{
	//效验状态
	//ASSERT((m_cbDownLoadStatus==DOWN_LOAD_IDLE)||(m_cbDownLoadStatus==DOWN_LOAD_ERROR));
	//if ((m_cbDownLoadStatus!=DOWN_LOAD_IDLE)&&(m_cbDownLoadStatus!=DOWN_LOAD_ERROR)) return false;
	...
	CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
	TCHAR szDownLoadUrl[MAX_PATH]=TEXT("");
	//_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("http://%s/DownLoad.aspx?KindID=%ld"),szDownloadIP,wKindID);
	_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("http://%s/DownLoad/%s"),szDownloadIP,szProcessName);
     m_sDownLoadUrl  = szDownLoadUrl;
	//发送状态
	m_pDlgDownLoad->SendMessage(WM_ITEM_STATUS_UPDATE,(WPARAM)this,0);
	//执行下载
...
//下载异常
bool CDownLoadSink::OnDownLoadError(enDownLoadError DownLoadError,INT nErrorCode,CString sDescrible)
{
	//设置变量
	m_cbDownLoadStatus=DOWN_LOAD_ERROR;
	...
	m_pDlgDownLoad->SendMessage(WM_ITEM_STATUS_UPDATE,(WPARAM)this,0);
	CString sErrorMsg;
	sErrorMsg.Format("%s  下载失败,%d,%s",m_sDownLoadUrl,nErrorCode,sDescrible);
	m_pDlgDownLoad->m_DownLogFile.Write(sErrorMsg);
...
bool CDownLoadSink::OnDownLoadStatus(enDownLoadStatus DownLoadStatus)
{
	...
	{
	case DownLoadStatus_Connect:		//链接成功
		{
			//设置状态
			m_cbDownLoadStatus=DOWN_LOAD_LINK_SUCESS;
			m_pDlgDownLoad->m_DownLogFile.Write(m_sDownLoadUrl + ":  连接成功");
	...
	case DownLoadStatus_Service:		//服务状态
		{
			...
			m_pDlgDownLoad->m_DownLogFile.Write(m_sDownLoadUrl+":  找到下载文件");
	...
	case DownLoadStatus_Conclude:		//完成状态
		{
			...
			m_pDlgDownLoad->m_DownLogFile.Write(m_sDownLoadUrl+":  下载完成");
...
bool CDownLoadSink::OnDataInformation(DWORD dwTotalFileSize, LPCTSTR pszEntityTag, LPCTSTR pszLocation)
{
	...
	CString sLogMsg;
	sLogMsg.Format("%s:  解析到下载文件大小 %d",m_sDownLoadUrl,m_dwTotalFileSize);
	m_pDlgDownLoad->m_DownLogFile.Write(sLogMsg);
...
bool CDownLoadSink::PerformInstall()
{
	...
	//变量定义
	INT nTailCount=lstrlen(TEXT(".XWD"));
...
bool CDownLoadSink::CreateDownFile(LPCTSTR pszLocation)
{
	...
	_sntprintf(szFilePath,CountArray(szFilePath),TEXT("%s\\%s.XWD"),szDownLoadPath,pszFileName);
...
VOID CDlgDownLoad::DoDataExchange(CDataExchange * pDX)
{
	...
	DDX_Control(pDX, IDC_DOWNLOAD_DX, m_btDownLoadDX);
	DDX_Control(pDX, IDC_DOWNLOAD_WT, m_btDownLoadWT);
	DDX_Control(pDX, IDC_STATIC_WEBDOWN, m_webDownload);
	DDX_Control(pDX, IDC_STATIC_WEBDOWN1, m_webDownload1);
}
...
BOOL CDlgDownLoad::OnInitDialog()
{
	...
	m_webDownload.SetCustomFont(true,false,false,false);
	m_webDownload1.SetCustomFont(true,true,false,false);

	m_webDownload.SetBackGroundColor(RGB(193,211,222));
	m_webDownload1.SetBackGroundColor(RGB(193,211,222));

	m_webDownload.SetHyperLinkTextColor(RGB(0,0,0),RGB(0,0,0),RGB(0,0,0));
	m_webDownload1.SetHyperLinkTextColor(RGB(162,107,50),RGB(162,107,50),RGB(162,107,50));

	m_webDownload.SetWindowText("若下载失败,请选择");
	m_webDownload1.SetWindowText("【官网下载】");
	//m_webDownload.ShowWindow(SW_HIDE);

	m_btLast.ShowWindow(SW_HIDE);
...
VOID CDlgDownLoad::OnCancel()
{
	...
	if (nResult==IDYES)
	{
		...
		if (pGlobalUserInfo->GetGlobalUserData()->dwUserID==0) 
...
VOID CDlgDownLoad::OnBnClickedLast()
{
	...
	m_btDownLoadDX.EnableWindow(FALSE);
	m_btDownLoadWT.EnableWindow(FALSE);
...
VOID CDlgDownLoad::OnBnClickedRetryAgain()
{
	...
	ASSERT(m_wFocusItem!=INVALID_WORD);
	ASSERT(m_DownLoadSinkArray.GetCount()>m_wFocusItem);
	if (m_DownLoadSinkArray.GetCount()<=m_wFocusItem)
	{
		return;
	}
	...
	//获取子项
	CDownLoadSink * pDownLoadSink=m_DownLoadSinkArray[m_wFocusItem];
	pDownLoadSink->PerformDownLoad(pDownLoadSink->m_szClientName,pDownLoadSink->m_wKindID,pDownLoadSink->m_wServerID,pDownLoadSink->m_sDownLoadFileName);

	return;
}

VOID CDlgDownLoad::OnBnClickedDX()
{
	//效验状态
	ASSERT(m_wFocusItem!=INVALID_WORD);
	ASSERT(m_DownLoadSinkArray.GetCount()>m_wFocusItem);
	...
	//获取子项
	CDownLoadSink * pDownLoadSink=m_DownLoadSinkArray[m_wFocusItem];
	pDownLoadSink->PerformDownLoad(pDownLoadSink->m_szClientName,m_sHostDX,pDownLoadSink->m_sDownLoadFileName);

}

VOID CDlgDownLoad::OnBnClickedWT()
{
	//效验状态
	ASSERT(m_wFocusItem!=INVALID_WORD);
	ASSERT(m_DownLoadSinkArray.GetCount()>m_wFocusItem);
	...
	//获取子项
	CDownLoadSink * pDownLoadSink=m_DownLoadSinkArray[m_wFocusItem];
	pDownLoadSink->PerformDownLoad(pDownLoadSink->m_szClientName,m_sHostWT,pDownLoadSink->m_sDownLoadFileName);

}

VOID CDlgDownLoad::OnBnClickedWebGameDownLowd()
{
	//构造地址
	CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
	TCHAR szWebDownLink[256]=TEXT("");
	_sntprintf(szWebDownLink,CountArray(szWebDownLink),TEXT("%s/DownLoadList.aspx"),pGlobalServer->GetPlatformServer(3));

	//打开页面
	ShellExecute(NULL,TEXT("OPEN"),szWebDownLink,NULL,NULL,SW_NORMAL);
}
...
VOID CDlgDownLoad::OnLButtonDown(UINT nFlags, CPoint Point)
{
	...
	if (m_wFocusItem!=m_wHoverItem)
	{
		...
		bool bEnable = VerdictRetryAgain(m_wFocusItem);
		m_btRetryAgain.EnableWindow((bEnable==true)?TRUE:FALSE);
		m_btDownLoadDX.EnableWindow((bEnable==true)?TRUE:FALSE);
		m_btDownLoadWT.EnableWindow((bEnable==true)?TRUE:FALSE);
	}
...
VOID CDlgDownLoad::OnWindowPosChanged(WINDOWPOS * lpwndpos)
{
	...
	if (m_bCreate==true)
	{
		...
		m_webDownload.SetWindowPos(NULL,10,nYControlPos+4,0,0,SWP_NOSIZE|SWP_NOZORDER);
		m_webDownload1.SetWindowPos(NULL,20+180-65,nYControlPos+4,0,0,SWP_NOSIZE|SWP_NOZORDER);
		...
		INT nSpace = rcButton.Width()+6;
		m_btCancelAll.SetWindowPos(NULL,rcClient.Width()-nSpace-nRBorder,nYButtonPos,0,0,SWP_NOSIZE|SWP_NOZORDER);
		m_btCancelOne.SetWindowPos(NULL,rcClient.Width()-nSpace*2-nRBorder,nYButtonPos,0,0,SWP_NOSIZE|SWP_NOZORDER);
		m_btRetryAgain.SetWindowPos(NULL,rcClient.Width()-nSpace*3-nRBorder,nYButtonPos,0,0,SWP_NOSIZE|SWP_NOZORDER);

		m_btDownLoadWT.SetWindowPos(NULL,rcClient.Width()-nSpace*3-nRBorder,nYButtonPos,0,0,SWP_NOSIZE|SWP_NOZORDER);
		m_btDownLoadDX.SetWindowPos(NULL,rcClient.Width()-nSpace*4-nRBorder,nYButtonPos,0,0,SWP_NOSIZE|SWP_NOZORDER);
	}
...
//下载游戏
bool CDlgDownLoad::DownLoadClient(LPCTSTR pszClientName, WORD wKindID, WORD wServerID,LPCTSTR szProcessName)
{
	//查找游戏
	for (INT_PTR i=0;i<m_DownLoadSinkArray.GetCount();i++)
	{
	...
	pDownLoadSink->PerformDownLoad(pszClientName,wKindID,wServerID,szProcessName);
...
bool CDlgDownLoad::InitDownLoad()
{
	CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
	m_sDownLogTxt.Format("%s/DownLoad/download.log",pGlobalServer->GetAppPath());
	m_DownLogFile.Init(m_sDownLogTxt);

	TCHAR szTelecom[LEN_SERVER]=TEXT("");
	pGlobalServer->ReadPlatFileString(TEXT("ServerInfo"),"Telecom",szTelecom,MAX_PATH);
	m_sHostDX = szTelecom;

	TCHAR szNetcom[LEN_SERVER]=TEXT("");
	pGlobalServer->ReadPlatFileString(TEXT("ServerInfo"),"Netcom",szNetcom,MAX_PATH);
	m_sHostWT = szNetcom;
	return true;
}
...
bool CDlgDownLoad::VerdictRetryAgain(WORD wFocuxIndex)
{
	...
	if (wFocuxIndex!=INVALID_WORD)
	{
		...
		//取消判断
		if (pDownLoadSink->m_cbDownLoadStatus==DOWN_LOAD_ERROR)
			return true;
	}
