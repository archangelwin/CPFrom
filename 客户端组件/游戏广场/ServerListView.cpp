...
#define ITEM_HEIGHT					1									//子项高度

#define ITEM_TYPE_HEIGHT			29									//子项高度
#define ITEM_KIND_HEIGHT			27									//子项高度
#define ITEM_ROOM_HEIGHT			21									//子项高度
#define ITEM_LAST_HEIGHT			80									//子项高度
...
CServerListView::CServerListView()
{
	...
	m_FontGameKind.CreateFont(-12,0,0,0,400,0,0,0,134,3,2,ANTIALIASED_QUALITY,2,TEXT("宋体"));
	m_FontGameServer.CreateFont(-12,0,0,0,400,0,0,0,134,3,2,ANTIALIASED_QUALITY,2,TEXT("宋体"));
...
VOID CServerListView::DrawTreeItem(CDC * pDC, CRect & rcClient, CRect & rcClipBox)
{
	...
	do
	{
		...
		if (GetItemRect(hItemCurrent,rcItem,TRUE))
		{
			...
			if ((pGameListTemp!=NULL)&&(pGameListTemp->GetItemGenre()==ItemGenre_Server)) //游戏房间
			{
				...
				else
				{
					...
					if(_tcscmp(szLoadInfo,_T("闲")) == 0)
						pDC->SetTextColor(RGB(117,199,0));
					else if(_tcscmp(szLoadInfo,_T("良")) == 0)
						pDC->SetTextColor(RGB(16,185,240));
					else if(_tcscmp(szLoadInfo,_T("忙")) == 0)
						pDC->SetTextColor(RGB(243,140,1));
					else if(_tcscmp(szLoadInfo,_T("挤")) == 0)
						pDC->SetTextColor(RGB(167,30,224));
					else if(_tcscmp(szLoadInfo,_T("满")) == 0)
			...
			else if ((pGameListTemp!=NULL)&&(pGameListTemp->GetItemGenre()==ItemGenre_Kind)) //游戏种类
			{
			...
			else if ((pGameListTemp!=NULL)&&(pGameListTemp->GetItemGenre()==ItemGenre_Type)) //游戏类别（热门、等）
			{
...
LPCTSTR CServerListView::GetLoadInfo(DWORD dwOnLineCount, DWORD dwMaxCount, LPTSTR pszBuffer, WORD wBufferSize)
{
	...
	if(dwPer > 80)
	{
		_sntprintf(pszBuffer, wBufferSize, TEXT("满"));
	}
	else if(dwPer > 60)
	{
		_sntprintf(pszBuffer, wBufferSize, TEXT("挤"));
	}
	else if(dwPer > 40)
	{
		_sntprintf(pszBuffer, wBufferSize, TEXT("忙"));
	}
	else if(dwPer > 20)
	{
		_sntprintf(pszBuffer, wBufferSize, TEXT("良"));
	}
	else
	{
		_sntprintf(pszBuffer, wBufferSize, TEXT("闲"));
	}
