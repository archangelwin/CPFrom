...
void CMissionLogon::SetLoginSocketType(int SocketType)
{
	m_LogonSocketType = SocketType;
}

void CMissionLogon::SetHaveLogonIn(bool bHaveLogonIn)
{
	m_bHaveLoginIn = bHaveLogonIn;
}
//取消连接
...
{
	if (m_LogonSocketType>0) //1退出登录,2切换账号
	{
		if(nErrorCode!=0L) //连接错误
		{
			//退出客户端
			CPlatformFrame * pPlatformFrame=CPlatformFrame::GetInstance();
			if (pPlatformFrame!=NULL) 
				pPlatformFrame->ClosePazza(m_LogonSocketType); //关闭客户端
		}
		else
		{
			WORD wPacketSize;
			...
			if(m_bLowVerValidate)
				cbValidateFlags = MB_VALIDATE_FLAGS|LOW_VER_VALIDATE_FLAGS;
			else
				cbValidateFlags = MB_VALIDATE_FLAGS;
			...
			//登录信息
			pLogonGameID->dwGameID=pGlobalUserInfo->GetGlobalUserData()->dwGameID;
			lstrcpyn(pLogonGameID->szPassword,pGlobalUserInfo->GetGlobalUserData()->szPassword,CountArray(pLogonGameID->szPassword));
			pLogonGameID->cbValidateFlags=cbValidateFlags;

			wPacketSize =  sizeof(CMD_GP_LogonGameID);
			//发送数据
			ASSERT(GetMissionManager()!=NULL);
			GetMissionManager()->SendData(MDM_GP_LOGON,SUB_GP_LOGON_LOGOUT_GAMEID,cbBuffer,wPacketSize);
		}
		//CMissionManager * pMissionManager=GetMissionManager();
		//if (pMissionManager!=NULL) 
		//	pMissionManager->ConcludeMissionItem(this,false);
	...
	//错误处理
	else if (nErrorCode!=0L)
	{
		//重试任务
		//if (nErrorCode!=SHUT_REASON_NORMAL)
		//CMissionManager * pMissionManager=GetMissionManager();
...
bool CMissionLogon::OnEventMissionRead(TCP_Command Command, VOID * pData, WORD wDataSize)
{
	if (Command.wMainCmdID==MDM_GP_LOGON)
	{
		...
		case SUB_GP_LOGON_LOGOUT_GAMEID:
			{
				return OnSocketSubClosePazza(pData,wDataSize);
			}
		}
	}
...
VOID CMissionLogon::UpdateUserInfo()
{
	...
	if (m_bRegister || m_pDlgLogon->m_cbAccountType==0)
	{
		RegUserItem.WriteString(TEXT("AccountType"),"0");
		pGlobalUserInfo->SetUserAccountType(0);
	}
	else
	{
		CString sAccountType;
		sAccountType.Format("%d",m_pDlgLogon->m_cbAccountType);
		RegUserItem.WriteString(TEXT("AccountType"),sAccountType);
		pGlobalUserInfo->SetUserAccountType(m_pDlgLogon->m_cbAccountType);
	}
...
bool CMissionLogon::OnSocketSubLogonSuccess(VOID * pData, WORD wDataSize)
{
	...
	{
		pGlobalServer->SetLastServerName(m_pDlgLogon->m_szLogonServer);
	}
	...
	pGlobalUserData->wFaceID=wFaceID/*pLogonSuccess->wFaceID*/;
	...
	lstrcpyn(pIndividualUserData->szMobilePhone,pLogonSuccess->szPhoneVerifyID,CountArray(pIndividualUserData->szMobilePhone));
...
// 	if(pLogonFailure->lResultCode == 8 ||pLogonFailure->lResultCode == 9)
// 	{
// 		CDlgPassValidate *pDlgPassValidate = new CDlgPassValidate;
// 		pDlgPassValidate->SetValidateType(pLogonFailure->lResultCode);
// 		if (pDlgPassValidate->DoModal() == IDOK)
// 		{
// 			SendLogonPacket(0,pLogonFailure->lResultCode, pDlgPassValidate->m_sValidateCode);
// 			return true;
// 		}
// 
// // 		CAuthenDlg *pDlgAuthen = new CAuthenDlg;
// // 		pDlgAuthen->DoModal();
// 	}

	if(pLogonFailure->lResultCode == 9)
	{
		CAuthenDlg *pDlgAuthen = new CAuthenDlg;
		if (pDlgAuthen->DoModal() == IDOK)
		{
			SendLogonPacket(0,pLogonFailure->lResultCode, pDlgAuthen->m_sValidateCode);
			return true;
		}else
			return true;
	}
	...
	{
		if (ShowInformation(pszDescribeString,MB_ICONERROR,60) == IDOK && pLogonFailure->lResultCode == 10)
		{
			CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
			TCHAR szSeiviceUrl[MAX_PATH]=TEXT("");
			_sntprintf(szSeiviceUrl,CountArray(szSeiviceUrl),TEXT("%s/Service/IssueList.aspx"),pGlobalServer->GetPlatformServer(3));

			ShellExecute(NULL, TEXT("OPEN"),szSeiviceUrl,NULL,NULL,SW_NORMAL);
		}
	}
...
bool CMissionLogon::OnSocketSubLogonFinish(VOID * pData, WORD wDataSize)
{
	...
	//CPlatformFrame::GetInstance()->WebBrowse(pGlobalServer->GetPlatformServer(3),false);
...
bool CMissionLogon::OnSocketSubUpdateNotify(VOID * pData, WORD wDataSize)
{
	...
	if (pGlobalUnits!=NULL)
		pGlobalUnits->DownLoadClient(TEXT("游戏大厅"),0,0,"lmy.exe");

	return true;
}

//发送信息
bool CMissionLogon::SendLogonPacket(BYTE cbValidateFlags,int nValidateType,CString sCode)
{
	if (m_pDlgLogon==NULL) 
		return false;
	...
	if (m_bRegister==false)
	{
		wPacketSize=m_pDlgLogon->ConstructLogonPacket(cbBuffer,sizeof(cbBuffer), cbValidateFlags,nValidateType,sCode);
...
bool CMissionLogon::OnSocketSubClosePazza(VOID * pData, WORD wDataSize)
{
	CMissionManager * pMissionManager=GetMissionManager();
	if (pMissionManager!=NULL) 
		pMissionManager->ConcludeMissionItem(this,false);
