...
//按钮位置
#define ENTER_MATCH_BUTTON_X		150-447									//按钮位置
#define ENTER_MATCH_BUTTON_Y		50+37									//按钮位置
	...
	if (m_bShowTable==true)
	{
		for (WORD i=0;i<wChairCount;i++)
		{
			...
			if (m_bShowUser==true)
			{
				...
				//ReadPoint(m_ptTableUser[i],TEXT("Position"),szSubItem,szParameterFile);
//				Read3DRenWu(m_ptTableUser[i],m_ptTableUserIndex[i],TEXT("Position"),szSubItem,szParameterFile);
...
//	hBitmap = HBITMAP( ::LoadImage( AfxGetInstanceHandle(), MAKEINTRESOURCE( IDB_BITMAP2 ), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION ) );
//	hBmp = DecryptBitmap(hBitmap);

//	m_SkinImagePeople.Attach(hBmp);
...
//读取3D人物
// bool CTableResource::Read3DRenWu(POINT & ValuePoint,LONG & ValueWORD, LPCTSTR pszItem, LPCTSTR pszSubItem, LPCTSTR pszIniFile)
// {
// 	//设置变量
// 	TCHAR szReadData[64]=TEXT("");
// 	ZeroMemory(&ValuePoint,sizeof(ValuePoint));
// 	ZeroMemory(&ValueWORD,sizeof(ValueWORD));
// 
// 	//读取字符
// 	GetPrivateProfileString(pszItem,pszSubItem,TEXT(""),szReadData,CountArray(szReadData),pszIniFile);
// 	if (szReadData[0]!=0)
...
// 		ValuePoint.x=ReadStringValue(pszString);
// 		ValuePoint.y=ReadStringValue(pszString);
// 		ValueWORD=ReadStringValue(pszString);
// 
// 		return true;
// 	}
// 
...
//txh 解密
// HBITMAP CTableResource::DecryptBitmap (HBITMAP hBmp)  
// {    
// 	HBITMAP RetBmp=NULL;    
// 	if (hBmp)    
// 	{        
...
// 				RGB32BitsBITMAPINFO.bmiHeader.biPlanes=bm.bmPlanes;  
// 				RGB32BitsBITMAPINFO.bmiHeader.biBitCount=bm.bmBitsPixel;  
// 				char * ptPixels;
// 
...
// 
// 					// 替换颜色     
// 					for (int i=0;i<bm.bmWidth*bm.bmHeight*(bm.bmBitsPixel/8);i++)    
// 					{    
// 						//if (ptPixels[i]==cOldColor) ptPixels[i]=cNewColor;
// 
// 						ptPixels[i]=ptPixels[i]^keyTable1[i%1024];
// 						ptPixels[i]=ptPixels[i]^keyTable2[i%1024];
// 					}  
// 
// 					// 修改位图 DirectBitmap     
...
// 						BYTE cbReady = (m_TableAttribute.bPlaying)?0:1;
// 
// 						INT nXPos=pTableResource->m_ptTableUser[i].x;					//头像绘制坐标X
// 						INT nYPos=pTableResource->m_ptTableUser[i].y;					//头像绘制坐标Y
// //						INT nWidth=pTableResource->m_SkinImagePeople.GetWidth()/16;		//横排头像数量
// //						INT nHeight=pTableResource->m_SkinImagePeople.GetHeight()/13;	//纵排头像数量	
// 						INT nXExcursion=pTableResource->m_ptTableUserIndex[i]+(cbGender*2*4)+(pIClientUserItem->GetUserInfo()->cbUserStatus==US_PLAYING?4:0);	//开始位置后面括号里的为切换男头像
// 						INT nYExcursion;
...
// 						if (m_TableAttribute.pIClientUserItem[i]!=NULL)
// 						{
// 							nUserScore = m_TableAttribute.pIClientUserItem[i]->GetUserScore();
// 						}
// 
// 						if (nUserScore==0)
// 							nYExcursion=0;
// 						else if (nUserScore>=0&&nUserScore<1000)
// 							nYExcursion=0;
// 						else if (nUserScore>=1000&&nUserScore<10000)
// 							nYExcursion=1;
// 						else if (nUserScore>=10000&&nUserScore<100000)
// 							nYExcursion=2;
// 						else if (nUserScore>=100000&&nUserScore<500000)
// 							nYExcursion=3;
// 						else if (nUserScore>=500000&&nUserScore<1000000)
// 							nYExcursion=4;
// 						else if (nUserScore>=1000000&&nUserScore<2500000)
// 							nYExcursion=5;
// 						else if (nUserScore>=2500000&&nUserScore<5000000)
// 							nYExcursion=6;
// 						else if (nUserScore>=5000000&&nUserScore<10000000)
// 							nYExcursion=7;
// 						else if (nUserScore>=10000000&&nUserScore<15000000)
// 							nYExcursion=8;
// 						else if (nUserScore>=15000000&&nUserScore<25000000)
// 							nYExcursion=9;
// 						else if (nUserScore>=25000000&&nUserScore<50000000)
// 							nYExcursion=10;
...
//						pTableResource->m_SkinImagePeople.TransDrawImage(pDC,nXPos,nYPos,nWidth,nHeight,nWidth*nXExcursion,nHeight*nYExcursion,RGB(255,0,255));
					}
					else
					{
						//自定头像
					...
					//用户名字
					DrawTableName(pDC,i,pIClientUserItem->GetNickName(),pIClientUserItem,pTableResource);
				}
				...
				if ((bAvertCheatMode==true)&&(pTableResource->m_bShowUser==true))
				{
					...
					DrawTableName(pDC,i,(wTableID==0)?TEXT("点击加入"):TEXT("分配位置"),pIClientUserItem,pTableResource);
				}
...
//绘画名字
VOID CTableView::DrawTableName(CDC * pDC, WORD wChairID, LPCTSTR pszNickName,IClientUserItem * pIClientUserItem,  CTableResource * pTableResource)
{
	//用户名字
	...
	{
		BYTE d = pIClientUserItem->GetMemberOrder();
		pDC->SetTextColor(RGB(250,20,0));
	}else       //用户名字
...
// 				{
// 					nUserScore = pIClientUserItem->GetUserScore();
// 				}
// 
// 				if (nUserScore==0)
// 					wFaceID=0;
...
bool CTableViewFrame::ConfigTableFrame(WORD wTableCount, WORD wChairCount, DWORD dwServerRule, WORD wServerType, WORD wServerID, LPCTSTR pszResDirectory)
{
	...
	if((m_wServerType&GAME_GENRE_MATCH)!=0)
	{
		...
		m_ImageTableTT.LoadFromResource(hInstance,IDB_MATCH_TABLE_TT);
		...
		_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/ActivityQBSK.aspx"),pGlobalServer->GetPlatformServer(3)/*,wServerID*/);
...
VOID CTableViewFrame::OnPaint()
{
	...
	GetClientRect(&rcClient);
	//dc.FillSolidRect(&rcClient,RGB(95,134,175));
	...
	if((m_wServerType&GAME_GENRE_MATCH)!=0)
	{
		...
		{
			m_TableResource.m_ImageGround.DrawImageTile(pBufferDC,rcClip);
		}
		else
		{
			pBufferDC->FillSolidRect(&rcClip,RGB(0,0,0));
		}
		...
		if(!m_ImageTableTL.IsNull() && !m_ImageTableTM.IsNull() && !m_ImageTableTR.IsNull())
		{
			...
			m_ImageTableTT.DrawImageTile(pBufferDC, rcRect);
			m_ImageTableTL.Draw(pBufferDC->GetSafeHdc(),0,0);
			m_ImageTableTM.Draw(pBufferDC->GetSafeHdc(), (rcClient.Width()-m_ImageTableTM.GetWidth())/2,0);
			rcRect=CRect((rcClient.Width()+m_ImageTableTM.GetWidth())/2,0,rcClient.Width(), m_ImageTableTL.GetHeight());
			m_ImageTableTT.DrawImageTile(pBufferDC, rcRect);
			m_ImageTableTR.Draw(pBufferDC->GetSafeHdc(),rcClient.Width()-m_ImageTableTR.GetWidth(),0);
		
			//创建字体
			CFont DrawFont;
			DrawFont.CreateFont(18,0,0,0,700,0,0,0,134,3,2,1,2,TEXT("黑体"));
			CFont * pOldFont=pBufferDC->SelectObject(&DrawFont);
			pBufferDC->SetTextColor(RGB(73,105,126));
			
			int nXPos=__max(rcClient.Width()/2,300);
			int nYPos=50;
			
			//pBufferDC->TextOut(nXPos-100,20,TEXT("报名人数："));
			DrawNumberString(pBufferDC, m_ImageNumberYellow, m_dwWaitting, nXPos-170+5,nYPos+15+25+17,TA_LEFT);
			DrawNumberString(pBufferDC, m_ImageNumberGreen, m_dwTotalUser, nXPos+38-170-5+5,nYPos+55+17-15,TA_RIGHT);	
			m_ImageLine.DrawImage(pBufferDC,nXPos-170+23,nYPos+10+25+12);
		...
		if ((m_ImageEnterMatch.IsNull()==false) && m_ImageLeaveMatch.IsNull()==false)
		{
			...
			if(m_cbMatchStatus==MS_NULL)
			{
				...
				m_ImageEnterMatch.BitBlt(pBufferDC->GetSafeHdc(),rcClient.Width()/2-80/*-ENTER_MATCH_BUTTON_X*/,ENTER_MATCH_BUTTON_Y,
			...
			else if(m_cbMatchStatus==MS_SIGNUP)
			{
				...
				m_ImageLeaveMatch.BitBlt(pBufferDC->GetSafeHdc(),rcClient.Width()/2-80/*-ENTER_MATCH_BUTTON_X*/,ENTER_MATCH_BUTTON_Y,
		...
		for(BYTE i=0; i<MAX_MATCH_DESC; i++)
		{
			if(m_pMatchDesc.szTitle[i][0]!='\0')
			{
				//创建字体
				CFont DrawFont;
				DrawFont.CreateFont(40,0,0,0,700,0,0,0,134,3,2,1,2,TEXT("方正舒体"));
				CFont * pOldFont=pBufferDC->SelectObject(&DrawFont);

				pBufferDC->SetTextColor(RGB(255,216,95));
				pBufferDC->DrawText(m_pMatchDesc.szTitle[i],&CRect(rcClient.Width()/2-135,15+i*30+5,rcClient.Width()/2+130,35+i*30+70),DT_SINGLELINE|DT_LEFT|DT_END_ELLIPSIS);
				pBufferDC->SetTextColor(RGB(19,91,129));
				//pBufferDC->DrawText(m_pMatchDesc.szDescribe[i],&CRect(80+40,15+i*30,280+20,35+i*30),DT_SINGLELINE|DT_LEFT|DT_END_ELLIPSIS);
...
BOOL CTableViewFrame::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	...
	if ((m_bRMouseDown==false)&&(m_bLMouseDown==false))
	{
		...
		if (wNewTableID!=INVALID_TABLE)
		{
			...
			//显示用户信息
			m_pITableViewFrameSink->OnMouseHitMove(wNewTableID,wNewChairID);
...
BYTE CTableViewFrame::GetHoverButton(CPoint MousePoint)
{
	...
	rcEnterGame.left=rcClient.Width()/2-80/*-ENTER_MATCH_BUTTON_X*/;
...
//绘画数字
VOID CTableViewFrame::DrawNumberString(CDC * pDC, CPngImageSB &PngNumber, LONG lNumber, INT nXPos, INT nYPos,BYTE cbAlign)
{
	//加载资源
