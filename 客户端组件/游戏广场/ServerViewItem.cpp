...
//常量定义
#define PROPERTY_CY					53									//游戏道具
#define PUBLICIZE_CY				0									//游戏广告
...
//常量定义
#define SPLITTER_CY					2									//拆分高度
#define SPLITTER_CX					0									//拆分宽度
#define TABLE_BUTTON_SX				7									//按钮空隙
...
#define IDC_WEB_MESSAGE             104                                 //消息控件

//时间标识
#define IDI_LOGON_ROOM			   11									//进入房间
#define IDI_DETECT_SOCKET		   12									//网络监测
...
CServerViewItem::CServerViewItem()
{
	...
	m_bCheckInScore=false;
	m_bOpenInsureDlg = false;
	m_bReconnect = false;
...
CServerViewItem::~CServerViewItem()
{
	...
	{
		SafeDelete(m_pDlgInsureServer);
		m_pDlgInsureServer = NULL;
	}
	KillTimer(IDI_DETECT_SOCKET);

	if (m_pUserInfoDlg!=NULL)
	{
		SafeDelete(m_pUserInfoDlg);
		m_pUserInfoDlg = NULL;
	}
	return;
}
...
//	DDX_Control(pDX, IDC_VIEW_CONTROL, m_btControl);
	...
	DDX_Control(pDX,IDC_BT_DAYSIGN_IN,m_btDaySignIn);
	DDX_Control(pDX,IDC_BT_EXCHANGE,m_btExChange);
	return;
}
...
//	m_GamePropertyCtrl.Create(NULL,NULL,WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN,rcCreate,this,IDC_GAME_PROPERTY);
	...
	m_PlatMessage.Create(NULL,NULL,WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN,rcCreate,this,IDC_WEB_MESSAGE);
	...
	m_btTableButton2.ShowWindow(SW_HIDE);
	m_btTableButton5.ShowWindow(SW_HIDE);

	m_btDaySignIn.SetButtonImage(IDB_BT_DAYSIGN,hInstance,true,false);
	...
	//CString sServerStatus;
	//sServerStatus.Format("%s/DownLoad/ServerStatus.log",pGlobalServer->GetAppPath());
	//m_ServerStatusLog.Init(sServerStatus);
	m_ChatEdit.ShowWindow(SW_HIDE);
	m_btChatShort.ShowWindow(SW_HIDE);
	m_btSendChat.ShowWindow(SW_HIDE);
	m_btChatButton1.ShowWindow(SW_HIDE);
	m_btChatButton2.ShowWindow(SW_HIDE);
	m_btChatButton3.ShowWindow(SW_HIDE);
	m_btChatButton4.ShowWindow(SW_HIDE);
	...
	SetTimer(IDI_DETECT_SOCKET,5*1000,NULL);
	return TRUE;
}
...
BOOL CServerViewItem::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_BT_CHAT_SHORT:			//聊天短语
		{
			...
			_sntprintf(szFileName,CountArray(szFileName),TEXT("%s\\LobbyDate\\PhraseInfo.ini"),szDirectory);
	...
	case IDC_BT_EXCHANGE:     //蓝钻
		{
			CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
			TCHAR szBrowser[MAX_PATH]=TEXT("");
			_sntprintf(szBrowser,CountArray(szBrowser),TEXT("%s/Shop/OnLineMall.aspx?type=1"),pGlobalServer->GetPlatformServer(3));

			CMessagePop DlgMobileBind ;
			DlgMobileBind.SetWindowSize(500,390,500,/*210-107+100*/350);
			DlgMobileBind.InitWebDlg(szBrowser,"游戏商城",true,false,0);
			//CDlgMobileBind DlgMobileBind;
			DlgMobileBind.DoModal();

// 			CDlgBrowser DlgBrowser;
// 			DlgBrowser.CreateBrowserControl(TEXT("蓝钻"),szBrowser,CSize(500,350));
// 			CInformation Information(this);
// 			Information.ShowMessageBox(TEXT("即将开放，敬请期待！"),MB_ICONINFORMATION,10);

			return TRUE;
		}
	case IDC_BT_DAYSIGN_IN:    //红钻
		{
			CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
			TCHAR szBrowser[MAX_PATH]=TEXT("");
			_sntprintf(szBrowser,CountArray(szBrowser),TEXT("%s/Shop/OnLineMall.aspx?type=2"),pGlobalServer->GetPlatformServer(3));
...
// 			CDlgBrowser DlgBrowser;
// 			DlgBrowser.CreateBrowserControl(TEXT("红钻"),szBrowser,CSize(500,350));
// 			m_TCPSocketModule->SendData(MDM_GR_USER,SUB_GR_CHECK_IN_GET_SCORE);
// 			m_bCheckInScore=true;
	...
	case IDM_GAME_RULE:				//游戏规则
		{
			...
			CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
			TCHAR szBrowser[MAX_PATH]=TEXT("");
			_sntprintf(szBrowser,CountArray(szBrowser),TEXT("%s/GameRule.aspx?GameID=%ld&KindID=%ld"),pGlobalServer->GetPlatformServer(3),m_GameKind.wGameID,
	...
	case IDM_GAME_CHART:			//游戏排行
		{
			...
			_sntprintf(szBrowser,CountArray(szBrowser),TEXT("%s/GameChart.aspx?GameID=%ld&KindID=%ld&ServerID=%ld&UserID=%ld"),pGlobalServer->GetPlatformServer(3),
	...
	case IDM_DELETE_SERVER_ITEM:	//关闭房间
		{
			...
			{
				m_TCPSocketModule->CloseSocket(SHUT_REASON_NORMAL);
...
{
	//动作过虑
	if(m_ServiceStatus == ServiceStatus_NetworkDown) //服务中断
	{
		m_ChatMessage.InsertSystemString(TEXT("与游戏服务器的连接中断了"));
		m_ChatMessage.InsertSystemString(TEXT("与游戏服务器重连中..."));
		...
		//int nRet = Information.ShowMessageBox(TEXT("该房间服务已中断,请点击确认关闭此房间"),MB_OKCANCEL);
		//if (nRet ==IDOK )
		//{
		//	if (m_TCPSocketModule->Connect(m_GameServer.szServerAddr,m_GameServer.wServerPort)!=CONNECT_SUCCESS)
		//	{
		...
		//	//CGameServerItem * pGameServerItem = new CGameServerItem;
		//	//ZeroMemory(pGameServerItem,sizeof(pGameServerItem));
		//	//InitServerViewItem(pGameServerItem,m_wAVServerPort,m_dwAVServerAddr,true);
		//	return true;
		//}
	...
	//绑定操作
	SCORE myTotalScore = m_pIMySelfUserItem->GetUserInsure()+m_pIMySelfUserItem->GetUserScore();
	...
	//if (myTotalScore > pCGlobalUnits->GetLockCoin() && !pGlobalUserData->cbMoorMachine)
	//{
	//	//构造提示
	//	CString sCoinStr;
	//	if (pCGlobalUnits->GetLockCoin() >10000)
	//	{
	//		sCoinStr.Format("您当前金币总额超过%dW，为了您的金币安全，请绑定本机再进行游戏，点击‘确认’进行绑定。",pCGlobalUnits->GetLockCoin()/10000);
	//	}
	//	else
	//	{
	//		sCoinStr.Format("您当前金币总额超过%d，为了您的金币安全，请绑定本机再进行游戏，点击‘确认’进行绑定。",pCGlobalUnits->GetLockCoin());
	//	}
	//	sCoinStr = "您当前金币总额过多，为了您的金币安全，请绑定本机再进行游戏，点击‘确认’进行绑定。";
	//	//退出判断
	//	CInformation Information(CPlatformFrame::GetInstance());
	...
	if (wChairID!=INDEX_ENTER_CHAIR) 
		pTableUserItem=m_TableViewFrame.GetClientUserItem(wTableID,wChairID);
...
//鼠标移动
VOID CServerViewItem::OnMouseHitMove(WORD wTableID, WORD wChairID)
{
	//移动位置
	if ((wTableID!=INVALID_TABLE)&&(wChairID!=INVALID_CHAIR)&&(wChairID!=INDEX_ENTER_CHAIR))
	{
		...
		if (pIClientUserItem!=NULL)
		{
			...
			//创建信息窗口
			if (m_pUserInfoDlg->m_hWnd == NULL)
			{
				m_pUserInfoDlg->Create(IDD_USERINFO_DLG,this);
			}
			...
			//获取等级描述
			LPCTSTR pszGameLevel = pGameLevelParser->GetLevelDescribe(pIClientUserItem);

			//设置信息
			m_pUserInfoDlg->UpdateUI(pIClientUserItem,pszGameLevel);
			...
			m_pUserInfoDlg->SetForegroundWindow();
	...
	{
		//隐藏窗口
		m_pUserInfoDlg->ShowWindow(SW_HIDE);
	}

}

//移动信息框
VOID CServerViewItem::MoveUserInfoDlg()
{
	CRect rcCleint;
	GetClientRect(&rcCleint);

	CRect rcUserInfoDlg;
	m_pUserInfoDlg->GetClientRect(rcUserInfoDlg);

	CPoint MousePoint;
	GetCursorPos(&MousePoint);
	int nX = MousePoint.x + 20;
	int nY = MousePoint.y + 20;
	if(nY + rcUserInfoDlg.Height() > rcCleint.bottom+120)
	{
		nY = nY - rcUserInfoDlg.Height();
	}
	//移动窗口
	m_pUserInfoDlg->SetWindowPos(NULL, nX, nY, 0, 0, SWP_NOSIZE|SWP_NOZORDER);
...
bool CServerViewItem::OnEventTCPSocketLink(WORD wSocketID, INT nErrorCode)
{
	...
	if (m_bReconnect)
	{
		m_ChatMessage.InsertSystemString(TEXT("与游戏服务器重连成功..."));
	}
	//KillTimer(IDI_DETECT_SOCKET);
...
bool CServerViewItem::OnEventTCPSocketShut(WORD wSocketID, BYTE cbShutReason)
{
	...
	if (cbShutReason!=SHUT_REASON_NORMAL)
	{
		if (m_ServiceStatus==ServiceStatus_ServiceIng)
		{
			...
			//m_ChatMessage.InsertSystemString(TEXT("与游戏服务器重新连接成功"));
			return true;
			//关闭提示
			//CInformation Information(this);
...
bool CServerViewItem::OnGameProcessClose(DWORD dwExitCode)
{
	...
	m_TableViewFrame.SetTableStatus(false);
...
// 			//发送判断
// 			//ccy20141208
// 			if (pIClientUserItem->GetTableID()==wTableID||m_pIMySelfUserItem->GetMatchNo()!=pIClientUserItem->GetMatchNo()||m_wServerType!=GAME_GENRE_MATCH) continue;
...
VOID CServerViewItem::OnUserItemUpdate(IClientUserItem * pIClientUserItem, tagUserScore & LastScore)
{
	...
	{
		//事件通知
		if(!CServerRule::IsTrainRoom(m_dwServerRule))
		{	
			//变量定义
			CGlobalUserInfo * pGlobalUserInfo=CGlobalUserInfo::GetInstance();
			tagGlobalUserData * pGlobalUserData=pGlobalUserInfo->GetGlobalUserData();
...
//			pGlobalUserData->dwRank=LastScore.dwRank;//ccy20141204
...
//		UserScore.dwRank=pUserInfo->dwRank;//ccy20141204
...
// 	_sntprintf(szMenuString,CountArray(szMenuString),TEXT("金币：") SCORE_STRING TEXT(" 经验值：%ld 逃跑率：%5.2f%%"),
// 		pUserInfo->lScore,pUserInfo->dwExperience,/*dwPlayCount,*/fUserFleeRate);

	CGlobalServer * pGlobalServer = CGlobalServer::GetInstance();
	if (pGlobalServer->BShowGameLevel() == true)
	{
	_sntprintf(szMenuString,CountArray(szMenuString),TEXT("金币：") SCORE_STRING TEXT(" 等级：%d 级"),
		pUserInfo->lScore,pUserInfo->dwExperience);
	}else
		_sntprintf(szMenuString,CountArray(szMenuString),TEXT("金币：") SCORE_STRING,
		pUserInfo->lScore);
...
//显示银行
VOID CServerViewItem::ShowInsureView(CString sRemInsurePwd)
{
	//效验参数
	ASSERT(m_pIMySelfUserItem!=NULL);
	...
	//禁用银行
	if(m_TableViewFrame.GetChairCount() >= MAX_CHAIR && m_pIMySelfUserItem->GetTableID()!=INVALID_TABLE)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("您已经在游戏中，请退出游戏再进行操作！"),MB_ICONERROR,30);
	...
	//_sntprintf(m_pDlgInsureServer->m_szSaveInsurePass,CountArray(m_pDlgInsureServer->m_szSaveInsurePass),sRemInsurePwd);
	////显示窗口
	//DlgInsureServer.DoModal();
	if (m_pDlgInsureServer==NULL/* ||m_pDlgInsureServer->m_hWnd==NULL */)
	{
		m_pDlgInsureServer = new CDlgInsureServer;
		m_pDlgInsureServer->Initialization(m_TCPSocketModule.GetInterface(),m_pIMySelfUserItem);	
		//_sntprintf(m_pDlgInsureServer->m_szSaveInsurePass,CountArray(m_pDlgInsureServer->m_szSaveInsurePass),sRemInsurePwd);
		m_pDlgInsureServer->SetInsurePass(sRemInsurePwd);
		//m_pDlgInsureServer = pDlgInsureServer;
		m_pDlgInsureServer->Create(424,this);
		m_pDlgInsureServer->CenterWindow();
		m_pDlgInsureServer->SetForegroundWindow();
		m_pDlgInsureServer->ShowWindow(SW_SHOW);
	}
	else
	{
		m_pDlgInsureServer->SetInsurePass(sRemInsurePwd);
		m_pDlgInsureServer->QueryUserInsureInfo();
		m_pDlgInsureServer->SwitchInsureModeView(1);
		m_pDlgInsureServer->CenterWindow();
		m_pDlgInsureServer->SetForegroundWindow();
		m_pDlgInsureServer->ShowWindow(SW_SHOW);
	}
...
bool CServerViewItem::OnSocketMainUser(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	case SUB_GR_CHECK_IN_GET_SCORE: //签到领币
		{
			if (m_bCheckInScore==true)
			{
				m_bCheckInScore=false;
				return OnCheckInScore(pData,wDataSize);
			}else
				return false;
	...
	case SUB_GR_CONFIG_SERVER:		//房间配置
		{
			...
			if (bSuccess==false)
			{
				...
				if (nResult==IDYES)
				{
					...
					pGlobalUnits->DownLoadClient(m_GameKind.szKindName,m_GameKind.wGameID,m_GameServer.wServerID,m_GameKind.szProcessName);
				}
...
//		UserInfo.dwRank=pGlobalUserData->dwRank;//ccy20141205
...
bool CServerViewItem::OnSocketSubLogonFinish(VOID * pData, WORD wDataSize)
{
	...
	CPlatformFrame * pPlatformFrame=CPlatformFrame::GetInstance();
	if (pPlatformFrame!=NULL) 
		pPlatformFrame->ActiveServerViewItem(this);
	if (pPlatformFrame!=NULL) 
		pPlatformFrame->AddLastPlayGame(m_GameServer.wServerID);
	...
	if ((m_pIMySelfUserItem!=NULL)&&(m_pIMySelfUserItem->GetTableID()!=INVALID_TABLE))
	{
		...
		if (bSuccess==false)
		{
			...
			m_ServiceStatus=ServiceStatus_NetworkDown;
			//m_ServerStatusLog.Write("CreateGameProcess");
		}
	}
	if (m_bOpenInsureDlg)
	{
...
bool CServerViewItem::OnSocketSubLogonSuccess(VOID * pData, WORD wDataSize)
{
	...
	_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/Games/RoomMsg.aspx"),pGlobalServer->GetPlatformServer(3));
	m_PlatMessage.Navigate(szBillUrl);
	m_PlatMessage.SetBoradColor(RGB(93,131,173));
...
bool CServerViewItem::OnSocketSubLogonFailure(VOID * pData, WORD wDataSize)
{
	...
	//m_ServerStatusLog.Write("OnSocketSubLogonFailure");
    CString sErrorMsg = pLogonFailure->szDescribeString;

	if(pLogonFailure->lErrorCode == 15 || pLogonFailure->lErrorCode == 16 ||pLogonFailure->lErrorCode == 17)
	{	//关闭处理
		m_TCPSocketModule->CloseSocket(SHUT_REASON_NORMAL);
		//显示消息
		LPCTSTR pszDescribeString=pLogonFailure->szDescribeString;
		CInformation Information;
		if(Information.ShowMessageBox(pszDescribeString,MB_ICONINFORMATION|MB_OKCANCEL,0) == IDOK)
		{			//显示窗口
			CDlgLockMachine DlgLockMachine;
			DlgLockMachine.DoModal();
		}
    ...
    else if(pLogonFailure->lErrorCode == 5) //同时进入多个房间
	{
		bool bPlaying = false; //是否正在游戏
		int nGameServerID = -1;  //游戏房间id
		CPlatformFrame * pPlatformFrame=CPlatformFrame::GetInstance();
		if (pPlatformFrame!=NULL) 
		{
			bPlaying = pPlatformFrame->GetPlayingGameServer(nGameServerID);
		}
		//显示消息
		LPCTSTR pszDescribeString=pLogonFailure->szDescribeString;
		if (lstrlen(pszDescribeString)>0) 
		{
			if (!bPlaying) //没有正在玩的游戏（强制进入、返回游戏）
			{
				if (ShowInformation(pszDescribeString,MB_ICONERROR,60,2) == IDOK) //强制进入
				{
					//关闭原来房间
					pPlatformFrame->ExitGameServer(nGameServerID); 
					//进入新房间
					SetTimer(IDI_LOGON_ROOM,100,NULL);
				}
				else //返回之前游戏
				{
					//关闭处理
					...
					PostMessage(WM_COMMAND,IDM_DELETE_SERVER_ITEM,0);
					//返回游戏
					pPlatformFrame->ActiveGameServe(nGameServerID);
				}
			}
			else //有正在玩的游戏（确认，返回游戏）
			{
				//关闭处理
				m_TCPSocketModule->CloseSocket(SHUT_REASON_NORMAL);
				if (ShowInformation(pszDescribeString,MB_ICONERROR,60,3) == IDOK) //确认
				{
					//关闭房间
					PostMessage(WM_COMMAND,IDM_DELETE_SERVER_ITEM,0);
				}
	...
	//更新信息
	else if(pLogonFailure->lErrorCode == LOGON_FAIL_SERVER_INVALIDATION) //房间失效
	{
		//关闭处理
		m_TCPSocketModule->CloseSocket(SHUT_REASON_NORMAL);
		//设置任务
		CPlatformFrame * pPlatformFrame=CPlatformFrame::GetInstance();
		if (pPlatformFrame!=NULL) 
			pPlatformFrame->ListGameServerInvalidation(m_GameKind.wKindID, m_GameServer.wServerID);
	}
	else if (pLogonFailure->lErrorCode==1)
	{
		//关闭处理
		m_TCPSocketModule->CloseSocket(SHUT_REASON_NORMAL);
		CInformation Information;
		if (Information.ShowMessageBox(pLogonFailure->szDescribeString,MB_ICONINFORMATION,30L,1) == IDOK)
		{
			//构造地址
			CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
...
bool CServerViewItem::OnSocketSubUpdateNotify(VOID * pData, WORD wDataSize)
{
	...
	if (pUpdateNotify->cbMustUpdatePlaza==TRUE)
	{
		...
		//下载大厅
		CGlobalUnits * pGlobalUnits=CGlobalUnits::GetInstance();
		if (pGlobalUnits!=NULL) pGlobalUnits->DownLoadClient(TEXT("游戏广场"),0,0,"lmy.exe");
	...
	if ((pUpdateNotify->cbMustUpdateClient==TRUE)||(pUpdateNotify->cbAdviceUpdateClient==TRUE))
	{
		...
		//更新提示
		CGlobalUnits * pGlobalUnits=CGlobalUnits::GetInstance();
		pGlobalUnits->DownLoadClient(m_GameKind.szKindName,m_GameKind.wKindID,m_GameServer.wServerID,m_GameKind.szProcessName);
	}
...
bool CServerViewItem::OnSocketSubRequestFailure(VOID * pData, WORD wDataSize)
{
	...
	if (pRequestFailure->lErrorCode == 1 || pRequestFailure->lErrorCode == 2)
	{
		CInformation Information;
		if (Information.ShowMessageBox(pRequestFailure->szDescribeString,MB_ICONINFORMATION,30L,1) == IDOK)
		{
			//构造地址
			CGlobalServer * pGlobalServer=CGlobalServer::GetInstance();
			ShellExecute(NULL, TEXT("OPEN"),pGlobalServer->GetPlatformServer(6),NULL,NULL,SW_NORMAL);
...
//等待分配
bool CServerViewItem::OnSocketSubWaitDistribute(VOID *pData, WORD wDataSize)
{
	//更新桌面
	m_TableViewFrame.SetTableStatus(true);
...
//签到领币
bool CServerViewItem::OnCheckInScore(VOID *pData, WORD wDataSize)
{
	//变量定义
	GP_CheckInGetSocre * pCheckInScore = (GP_CheckInGetSocre*)pData;
	WORD wHeadSize = sizeof(GP_CheckInGetSocre) - sizeof(pCheckInScore->szDescribeString);

	//效验参数
	ASSERT((wDataSize>wHeadSize)&&(wDataSize == (wHeadSize+strlen(pCheckInScore->szDescribeString)*sizeof(TCHAR))+1));
	if ((wDataSize<=wHeadSize)||(wDataSize!=(wHeadSize+strlen(pCheckInScore->szDescribeString)*sizeof(TCHAR))+1)) return false;

	//领币提示
	CInformation Information(this);
	Information.ShowMessageBox(pCheckInScore->szDescribeString,MB_ICONINFORMATION,10);
...
bool CServerViewItem::OnSocketSubSystemMessage(VOID * pData, WORD wDataSize)
{
	...
	if ((wType&(SMT_CLOSE_ROOM|SMT_CLOSE_LINK))!=0)
	{
		...
		//状态设置
		m_ServiceStatus=ServiceStatus_NetworkDown;
		//m_ServerStatusLog.Write("OnSocketSubSystemMessage");
	}
...
bool CServerViewItem::OnSocketSubActionMessage(VOID * pData, WORD wDataSize)
{
	...
	if ((pActionMessage->wType&(SMT_CLOSE_ROOM|SMT_CLOSE_LINK))!=0)
	{
		...
		//状态设置
		m_ServiceStatus=ServiceStatus_NetworkDown;
		//m_ServerStatusLog.Write("OnSocketSubActionMessage");
	}
...
VOID CServerViewItem::RectifyControl(INT nWidth, INT nHeight)
{
	...
	if (m_bRectifyHor==false)
	{
		...
		rcHorSplitter.bottom=rcHorSplitter.top+0/* SPLITTER_CY*/;
	}
	...
	rcChatArea.left=rcVorSplitter.right+2;
	rcChatArea.top=rcHorSplitter.bottom+/*PROPERTY_CY+*/95;
	...
	//调整拆分
	DeferWindowPos(hDwp,m_VorSplitter,NULL,rcVorSplitter.left,rcVorSplitter.top,rcVorSplitter.Width()+2,rcVorSplitter.Height(),uFlags);
	...
	DeferWindowPos(hDwp,m_btQuitServer,NULL,rcTableArea.right-rcQuitServer.Width()-3,3,0,0,uFlags|SWP_NOSIZE);

	//桌子按钮
	INT nXPosTable=rcTableArea.right-rcQuitServer.Width()-3-TABLE_BUTTON_SX;
	CSkinButton * TableButtonArray[]={&m_btTableButton4,&m_btTableButton3,&m_btTableButton1};
	...
	for (INT i=0;i<CountArray(TableButtonArray);i++)
	{
		...
		DeferWindowPos(hDwp,TableButtonArray[i]->m_hWnd,NULL,nXPosTable,3,0,0,uFlags|SWP_NOSIZE);
	}
	...
	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatArea.right-35+6,rcChatArea.bottom-19,0,0,uTargetFlag|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChatShort,NULL,rcChatArea.right-53+6,rcChatArea.bottom-19,0,0,uTargetFlag|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_ChatEdit,NULL,rcChatArea.left+5,rcChatArea.bottom-16,rcChatArea.Width()-59,12,uTargetFlag);
	...
	for (INT i=0;i<CountArray(ChatButtonArray);i++)
	{
		...
		DeferWindowPos(hDwp,ChatButtonArray[i]->m_hWnd,NULL,nXPosChat+2,nYPosChat+6,0,0,uFlags|SWP_NOSIZE|uTargetFlag);

		//设置间距
		nXPosChat+=(rcChatButton.Width()+nChatButtonSpace);
	}
//rcTableArea.top = rcTableArea.top+33;
	//环绕控件
	/*rcTableArea.top = 40;*/	
	DeferWindowPos(hDwp,m_PlatMessage,NULL,rcTableArea.left,27,rcTableArea.Width()-1,rcTableArea.top,uFlags);	
	...
	//移动控件(每日签到，兑换中心按钮)
	DeferWindowPos(hDwp,m_btDaySignIn,NULL,rcListArea.left+2,rcHorSplitter.bottom,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btExChange,NULL,(rcListArea.left+(rcListArea.Width()-5)/2)+3,rcHorSplitter.bottom,0,0,uFlags|SWP_NOSIZE);
	//DeferWindowPos(hDwp,m_PlatformPublicize,NULL,rcListArea.left,rcHorSplitter.bottom,rcListArea.Width()-5,PROPERTY_CY,uFlags);
...
//配置房间
VOID CServerViewItem::InitServerViewItem(CGameServerItem * pGameServerItem,WORD wAVServerPort,DWORD dwAVServerAddr,bool bReConnect,bool bOpenInsureDlg)
{
	//设置状态
	SetServiceStatus(ServiceStatus_Entering);
	m_bOpenInsureDlg = bOpenInsureDlg;
...
//	return false;
	...
	rcLeft.right = rcVorSplitter.left;
	rcLeft.top = rcClient.top;
	rcLeft.bottom = rcClient.bottom;

	rcRight.left = rcVorSplitter.right;
	rcRight.right = rcClient.Width();
	rcRight.top = rcClient.top;
	rcRight.bottom = rcClient.bottom;

	pDC->FillSolidRect(&rcLeft,RGB(95,134,175));
	pDC->FillSolidRect(&rcRight,RGB(30,129,186));
//	pDC->FillSolidRect(&rcRight,RGB(255,0,0));
	...
	//m_PlatMessage.ShowWindow(SW_HIDE);
	//m_UserListControl.ShowWindow(SW_HIDE);
	m_GamePropertyCtrl.ShowWindow(SW_HIDE);
	//m_ChatControl.ShowWindow(SW_HIDE);
	m_btDaySignIn.ShowWindow(SW_HIDE);
	m_btExChange.ShowWindow(SW_HIDE);
	...
	if (m_bHideUserList==false)
	{
		...
		rcChat.top=rcHorSplitter.bottom/*+PROPERTY_CY*/+95;
	...
	pDC->FillSolidRect(rcVorSplitter.right,0,rcClient.Width()-rcVorSplitter.right,PUBLICIZE_CY,RGB(255,0,0));
	...
	//建立缓冲
	CImage ImageBuffer;
	if(ImageBuffer.Create(rcClient.Width(),ImageTitleM.GetHeight(),32) == FALSE) return TRUE;
	...
	//填充中间
	for (INT nXPos=0;nXPos<rcVorSplitter.left;nXPos+=ImageTitleM.GetWidth())
	{
		ImageTitleM.BitBlt(BufferDC,nXPos,0);
	}
	//////////////////////////////////////////////////////////////////////////
	CPngImageSB m_ImgServerAdv;
	m_ImgServerAdv.LoadImage(AfxGetInstanceHandle(),TEXT("BK_SERVER_ADV"));
	m_ImgServerAdv.DrawImage(pDC,rcVorSplitter.right,rcHorSplitter.top-1);

	//绘画界面
	//pDC->BitBlt(0,0,rcVorSplitter.left,ImageTitleM.GetHeight(),pBufferDC,0,0,SRCCOPY);
...
//时间消息
VOID CServerViewItem::OnTimer(UINT_PTR nIDEvent)
{
	switch (nIDEvent)
	{
	case IDI_LOGON_ROOM:	//进入房间
		{
			KillTimer(IDI_LOGON_ROOM);
			SendLogonPacket();
			return;
		}
	}
