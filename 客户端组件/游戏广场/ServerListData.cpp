...
CServerListData::CServerListData()
{
	...
	m_pIServerListDataMatchSink = NULL;
	...
	m_GameKindInfoMap.InitHashTable(PRIME_KIND);
...
CServerListData::~CServerListData()
{
    ...
    //删除游戏类型
	CGameKindInfo *pKindInfo = NULL;
	Position=m_GameKindInfoMap.GetStartPosition();
	while (Position!=NULL)
	{
		m_GameKindInfoMap.GetNextAssoc(Position,wKey,pKindInfo);
		SafeDelete(pKindInfo);
	}
	m_GameKindInfoMap.RemoveAll();
...
//设置接口
VOID CServerListData::SetServerListDataSink(IServerListDataSink * pIServerListDataSink,IServerListDataSink * pIServerListDataMatchSink)
{
	//设置变量
	m_pIServerListDataSink=pIServerListDataSink;
	m_pIServerListDataMatchSink = pIServerListDataMatchSink;
...
bool CServerListData::InsertGameKind(tagGameKind * pGameKind)
{
	...
	CString sKindName = pGameKind->szKindName;
	//插入处理
	if (bInsert==true)
	{
		//设置索引
...
bool CServerListData::InsertGameServer(tagGameServer * pGameServer)
{
	...
	CString sKindName = pGameServerItem->m_pGameKindItem->m_GameKind.szKindName;
	bool bMatchRoom = false;
	if (sKindName.Find("比赛") != -1)  //比赛游戏
	{
		bMatchRoom = true;
	}
	...
	if(bInsert==true && pGameServerItem->m_pGameKindItem)
	{
		...
		//事件通知
		ASSERT(m_pIServerListDataSink!=NULL);
		if (m_pIServerListDataSink!=NULL && !bMatchRoom) 
	...
	if (bInsert==true)
	{
		...
		if (bMatchRoom)
		{
			m_pIServerListDataMatchSink->OnGameItemInsert(pGameServerItem);
		}
		else
		{
			m_pIServerListDataSink->OnGameItemInsert(pGameServerItem);
		}
		//if (pGameServerItem->m_GameServer.wServerType == 100)
		//{
		//}
	}
	else 
...
bool CServerListData::InsertGameKinfInfo(CGameKindInfo *pGameKindInfo)
{
	//效验参数
	ASSERT(pGameKindInfo!=NULL);
	if (pGameKindInfo==NULL) return false;

	//变量定义
	CGameKindInfo * pGameKindItem=NULL;
	bool bInsert=(m_GameKindInfoMap.Lookup(pGameKindInfo->nKindID,pGameKindItem)==FALSE);
	//获取存储
	if (bInsert==true)
	{
		//设置索引
		m_GameKindInfoMap[pGameKindInfo->nKindID]=pGameKindInfo;
		return true;
	}
	else
	{
		SafeDelete(pGameKindInfo);
		return false;
	}
}

//查找类型
CGameKindInfo * CServerListData::SearchGameKindInfo(WORD wKindID)
{
	CGameKindInfo * pGameKindInfo=NULL;
	m_GameKindInfoMap.Lookup(wKindID,pGameKindInfo);
	return pGameKindInfo;
}

void CServerListData::ResetDate()
{
	//变量定义
	WORD wKey=0;
