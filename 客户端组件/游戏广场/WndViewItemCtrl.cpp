...
#define IDC_BUTTON_SERVER_CLOSE			2000						    //房间关闭按钮
...
BEGIN_MESSAGE_MAP(CWndViewItemCtrl, CWnd)
	...
	ON_BN_CLICKED(IDC_BUTTON_SERVER_CLOSE, &CWndViewItemCtrl::OnBnClickedButtonColseServer)
...
//消息过虑
BOOL CWndViewItemCtrl::PreTranslateMessage(MSG * pMsg)
{
	//提示消息
...
//建立消息
INT CWndViewItemCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (__super::OnCreate(lpCreateStruct)==-1) return -1;
	CRect rcCreate(0,0,0,0);
	m_btClose.Create(NULL,WS_CHILD|WS_VISIBLE,rcCreate,this,IDC_BUTTON_SERVER_CLOSE);
	m_btClose.ShowWindow(SW_HIDE);
	HINSTANCE hInstance=AfxGetInstanceHandle();
	m_btClose.SetButtonImage(IDB_BT_CLOSE_SERVER,/*TEXT("BT_CLOSE_SERVER"),*/hInstance,true,false);
	//提示控件
	m_ToolTipCtrl.Create(this);
	m_ToolTipCtrl.Activate(TRUE);
	m_ToolTipCtrl.AddTool(&m_btClose,TEXT("关闭房间"));
...
// 	m_ToolTipCtrl.AddTool(&m_btClose,TEXT("关闭房间"));
	...
	{
		HINSTANCE hInstance=AfxGetInstanceHandle();
		m_btClose.SetButtonImage(IDB_BT_CLOSE_SERVER1/*,TEXT("BT_CLOSE_SERVER")*/,hInstance,true,false);
		...
		if (pViewItemInfo->szItemTitle[0]!=0)
		{
			if (i>0)
			{
				...
				if (pPlatformFrame!=NULL) 
				{
					...
					{
						m_nCurentGameServer = pServerViewItem->GetServerID();
					}		
				}
				//移动准备
				HDWP hDwp=BeginDeferWindowPos(32);
				UINT uFlags=SWP_NOACTIVATE|SWP_NOCOPYBITS|SWP_NOZORDER;
				DeferWindowPos(hDwp,m_btClose,NULL,PointButton.x+5,PointButton.y+2,0,0,uFlags|SWP_NOSIZE);
			...
			rcItemText.left=rcItemText.right=PointButton.x+43;
			int nTitleLen = lstrlen(pViewItemInfo->szItemTitle);
			if(i>0)
			{
				if (nTitleLen>9)
				{
				...
				{
					rcItemText.left=rcItemText.right=PointButton.x+42;
				}
			}
			else
			{
...
VOID CWndViewItemCtrl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	if ((m_wItemHover!=INVALID_WORD)&&(m_wItemHover==m_wItemDown))
	{
		...
		if (pViewItemInfo!=NULL) 
		{
			...
			//		if (pServerViewItem!=NULL)
			//		{
			//			pPlatformFrame->ExitGameServer(pServerViewItem->GetServerID());
			//		}		
			//	}
			//	return;
			//}
			//else
			//{
				ActiveViewItem(pViewItemInfo->pItemView);
			//}
		}
	}
...
void CWndViewItemCtrl::OnBnClickedButtonColseServer()
{
	//关闭房间
	CPlatformFrame * pPlatformFrame=CPlatformFrame::GetInstance();
	if (pPlatformFrame!=NULL) 
	{
		pPlatformFrame->ExitGameServer(m_nCurentGameServer);
	}	
}
