...
CDlgServicePassword::CDlgServicePassword() : CDlgServiceItem(IDD_DLG_SERVICE_PASSWORD)
{
	...
	ZeroMemory(m_szRemInsurePwd,sizeof(m_szRemInsurePwd));
...
VOID CDlgServicePassword::DoDataExchange(CDataExchange * pDX)
{
	...
	//
	DDX_Control(pDX, IDC_LOGONPWD_QQ, m_LogonPWD_QQ);
	DDX_Control(pDX, IDC_INSUREPWD_QQ, m_InsurePWD_QQ);
}
		...
		case SUB_GP_OPERATE_SUCCESS:	//操作成功
			{
				...
				else
				{
					...
					pPlatformFrame->SetRemInsurePwd(m_szRemInsurePwd);
				}
...
VOID CDlgServicePassword::OnBnClickedLogonPassword()
{
	...
	if (szSrcPassword[0]==0)
	{
		//提示信息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("请输入原账号密码！"),MB_ICONERROR,0);
...
VOID CDlgServicePassword::OnBnClickedInsurePassword()
{
	...
	if (szSrcPassword[0]==0)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("请输入原保险柜密码！"),MB_ICONERROR,0);
	...
	lstrcpyn(m_szSrcPassword,szSrcPassword,CountArray(m_szSrcPassword));
	lstrcpyn(m_szDesPassword,szDesPassword[0],CountArray(m_szDesPassword));
	lstrcpyn(m_szRemInsurePwd,szDesPassword[0],CountArray(m_szDesPassword));
...
BOOL CDlgServicePassword::OnInitDialog()
{
	__super::OnInitDialog();

	// TODO:  在此添加额外的初始化
	m_LogonPWD_QQ.SetHyperLinkTextColor(RGB(255,0,0),RGB(255,0,0),RGB(255,0,0));
	m_InsurePWD_QQ.SetHyperLinkTextColor(RGB(255,0,0),RGB(255,0,0),RGB(255,0,0));
	m_LogonPWD_QQ.SetBCursor(false);
	m_InsurePWD_QQ.SetBCursor(false);


	CGlobalUserInfo * pGlobalUserInfo=CGlobalUserInfo::GetInstance();
	if (pGlobalUserInfo->GetUserAccountType()>0)
	{
		m_edLogonPassword1.EnableWindow(false);
		m_edLogonPassword2.EnableWindow(false);
		m_edLogonPassword3.EnableWindow(false);
		m_btLogonPassword.EnableWindow(false);
	}
	else
	{
		m_InsurePWD_QQ.ShowWindow(SW_HIDE);
		m_LogonPWD_QQ.ShowWindow(SW_HIDE);
	}
