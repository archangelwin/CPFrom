...
VOID CDlgServiceBar::OnPaint()
{
	...
	CRect rcClient;
	GetClientRect(&rcClient);
	//dc.FillSolidRect(&rcClient,RGB(222,239,246));
	...
	//加载资源
	CPngImageSB ImageButton;
	CPngImageSB ImageBarLogo;
	CPngImageSB ImageBarImage;
	ImageButton.LoadImage(AfxGetInstanceHandle(),TEXT("SERVICE_BAR_BT"));
	...
	//pBufferDC->FillSolidRect(&rcClient,RGB(0,255,0));
	...
	for (INT i=0;i<m_DlgServiceItemArray.GetCount();i++)
	{
		...
		if ((m_wItemActive==i)||(m_wItemDown==i)) 
...
BOOL CDlgService::OnInitDialog()
{
	...
	m_DlgServiceBar.InsertServiceItem(&m_DlgBrowserPayRecord);

    m_DlgBrowserPayRecord.InitService(7,TEXT("充值记录"),TEXT("UserService/UserService3.aspx"));
	//移动窗口
	...
	if (pDlgServiceItem==NULL) 
		pDlgServiceItem=&m_DlgServiceIndividual;
...
CDlgServiceItem * CDlgService::GetServiceItem(BYTE cbServiceID)
{
	...
	switch (cbServiceID)
	{
	case SERVICE_INDIVIDUAL:			//基本资料
		{
			return &m_DlgServiceIndividual;
		}
	case SERVICE_PASSWORD:		        //密码保护
		{
			return &m_DlgServicePassword;
		}
	case SERVICE_PAY_RECORD:			//充值记录
		{
			return &m_DlgBrowserPayRecord;
		}
	}
