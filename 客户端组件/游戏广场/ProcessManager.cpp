...
#define TIME_DETECT_CREATE			20000L								//创建检测
#else
#define TIME_DETECT_NORMAL			5000L								//常规检测
#define TIME_DETECT_CREATE			15000L								//创建检测
#endif 
...
//进程数据
bool CProcessManager::OnProcessControlDatas(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize, HWND hWndSend)
{
	//效验状态
...
bool CProcessManager::CreateGameProcess(LPCTSTR pszProcessName)
{
	...
	if (m_ProcessControlModule.GetInterface()==NULL)
	{
		...
		//设置接口
		if (m_ProcessControlModule->SetProcessControlSinks(QUERY_ME_INTERFACE(IUnknownEx))==false)
		{
			ASSERT(FALSE);
			return false;
		}
