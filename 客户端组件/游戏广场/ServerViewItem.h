...
protected:
	...
	bool                            m_bCheckInScore;                    //签到标志
	bool                            m_bOpenInsureDlg;                   //进房间打开保险柜
	bool                            m_bReconnect;                       //重连判断
...
protected:
	...
	CUserInfoDlg *                  m_pUserInfoDlg;                      //用户信息窗口
...
//	CSkinButton						m_btControl;						//控制按钮
	...
	CSkinSplitter					m_VorSplitter;						//竖向拆分控件
	CSkinSplitter					m_HorSplitter;						//横向拆分控件
...
//	CPlatformPublicize				m_PlatformPublicize;				//浏览控件
	CPlatformPublicize              m_PlatMessage;                      //消息控件

	//游戏桌子
public:
	CSkinButton						m_btQuitServer;						//离开房间
	CSkinButton						m_btTableButton1;					//加入
	CSkinButton						m_btTableButton2;					//查找
	CSkinButton						m_btTableButton3;					//锁定
	CSkinButton						m_btTableButton4;					//设置
	CSkinButton						m_btTableButton5;					//更多
	CTableViewFrame					m_TableViewFrame;					//桌子框架

	CSkinButton                     m_btDaySignIn;                     //每日签到
	CSkinButton                     m_btExChange;                      //兑换中心 
...
public:
	...
	//鼠标移动
	virtual VOID OnMouseHitMove(WORD wTableID, WORD wChairID);
...
public:
	//显示银行
	VOID ShowInsureView(CString sRemInsurePwd);
	//老板事件
...
protected:
	...
	//签到领币
	bool OnCheckInScore(VOID *pData, WORD wDataSize);
...
public:
	...
	//配置房间
	VOID InitServerViewItem(CGameServerItem * pGameServerItem,WORD wAVServerPort,DWORD dwAVServerAddr,bool bReConnect = false,bool bOpenInsureDlg=false);
	//设置界面
...
public:
	tagGameServer GetGameServer(){ return m_GameServer;};
	tagGameKind GetGameKind(){ return m_GameKind;};
