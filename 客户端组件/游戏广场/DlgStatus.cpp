...
BOOL CDlgStatus::OnInitDialog()
{
	...
	SetWindowText(pGlobalServer->GetPlatformProduct(1));
	...
	//设置按钮
	m_btGiveUp.SetWindowPos(NULL,ImageStatus.GetWidth() - 23,2,0,0,SWP_NOSIZE|SWP_NOZORDER);
...
BOOL CDlgStatus::OnEraseBkgnd(CDC * pDC)
{
	...
	//绘画进度
	ImageStatusLine.DrawImage(pBufferDC,(ImageStatus.GetWidth()-SizeStatusLine.cx)/2,43,SizeStatusLine.cx,SizeStatusLine.cy,0,0);
	ImageStatusLine.DrawImage(pBufferDC,(ImageStatus.GetWidth()-SizeStatusLine.cx)/2,43,SizeStatusLine.cx*m_uImageIndex/100L,SizeStatusLine.cy,0,SizeStatusLine.cy);

	//绘画进度
	INT nXPoint=SizeStatusLine.cx*m_uImageIndex/100L;
	ImageStatusPoint.DrawImage(pBufferDC,(ImageStatus.GetWidth()-SizeStatusLine.cx)/2+nXPoint-SizeStatusPoint.cx/2,43+SizeStatusLine.cy/2-SizeStatusPoint.cy/2);
	//ImageStatusPoint.DrawImage(pBufferDC,(ImageStatus.GetWidth()-SizeStatusLine.cx)/2+nXPoint-SizeStatusPoint.cx/2,43/*+(SizeStatusLine.cy-SizeStatusPoint.cy)/2*/);

	//文字位置
	CRect rcDescribe;
	rcDescribe.SetRect((ImageStatus.GetWidth()-SizeStatusLine.cx)/2 +10 ,75,320,87);
