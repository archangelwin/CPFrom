...
public:
	...
	CPoint							m_ptTableUser[MAX_CHAIR];			//人物位置
	LONG							m_ptTableUserIndex[MAX_CHAIR];		//人物头像索引
	//框架位置
public:
...
public:
	...
	//外部资源
public:
	CPngImageSB						m_ImageEnter;						//进入位图
	CBitImage						m_ImageChair;						//椅子位图
	CBitImage						m_ImageTable;						//桌子位图
	CBitImage						m_ImageWaitDistribute;				//桌子位图
	CBitImage						m_ImageGround;						//背景位图
	CPngImageSB						m_ImageCustomUser;					//自定位图
//	CBitImage						m_SkinImagePeople;					//用户位图[性别0:女,1:男][状态0:举手,1:游戏中][桌子][衣服]
	//内部资源
public:
	CPngImageSB						m_ImageReady;						//准备位图
	CPngImageSB						m_ImageLocker;						//桌锁位图
	CPngImageSB						m_ImageNumberNB;					//号码位图
	CPngImageSB						m_ImageNumberBK;					//号码背景
	CBitImage						m_ImageUserFace;					//用户头像
//	HBITMAP                         hBitmap;                            //3D人物
//	HBITMAP                         hBmp;
...
protected:
	...
	//读取3D人物
//	bool Read3DRenWu(POINT & ValuePoint,LONG & ValueWORD, LPCTSTR pszItem, LPCTSTR pszSubItem, LPCTSTR pszIniFile);
	//解密
//	HBITMAP DecryptBitmap(HBITMAP hBmp);
};
...
public:
	...
	//绘画名字
	VOID DrawTableName(CDC * pDC, WORD wChairID, LPCTSTR pszNickName,IClientUserItem * pIClientUserItem, CTableResource * pTableResource);
	//绘画头像
...
public:
	...
	CBitImage                       m_ImageTableTT;
	CPngImageSB						m_ImageNumberGreen;					//绿色数字
	CPngImageSB						m_ImageNumberYellow;				//黄色数字
	CPngImageSB						m_ImageLine;						//图片资源
...
protected:
	...
	//绘画数字
	VOID DrawNumberString(CDC * pDC, CPngImageSB &PngNumber, LONG lNumber, INT nXPos, INT nYPos,BYTE cbAlign=TA_LEFT);
