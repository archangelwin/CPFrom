...
bool CSkinImage::DrawSkin( CDC* pDC, LPRECT lpRect, int xSrc, int ySrc, int nSrcWidth, int nSrcHeight, UINT uFlags, int left, int top, int right, int bottom, COLORREF crTransparent/*=RGB(255,0,255)*/ )
{
	...
	if( uFlags & DS_STRETCH_ALL )
	{
		CRect rcDest;
		CRect rcMem(lpRect);
		rcMem.OffsetRect(-lpRect->left, -lpRect->top);
		CMemDC memDC(pDC, &rcMem);

		if( uFlags & DS_TRANSPARENT )
			memDC.FillSolidRect(&rcMem, crTransparent);

		int nMode = memDC.SetStretchBltMode(HALFTONE);

		//画中间的
		rcDest.SetRect(rcMem.left + left, rcMem.top + top, rcMem.right - right, rcMem.bottom - bottom);
		::StretchBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), hDC,
			xSrc + left, ySrc + top, nSrcWidth - left - right, nSrcHeight - top - bottom, SRCCOPY);

		//画左边的
		if( left > 0 )
		{
			rcDest.SetRect(rcMem.left, rcMem.top + top, rcMem.left + left, rcMem.bottom - bottom);
			::StretchBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), hDC,
				xSrc, ySrc + top, left, nSrcHeight - top - bottom, SRCCOPY);
		}

		//画上边的
		if( top > 0 )
		{
			rcDest.SetRect(rcMem.left + left, rcMem.top, rcMem.right - right, rcMem.top + top);
			::StretchBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), hDC,
				xSrc + left, ySrc, nSrcWidth - left - right, top, SRCCOPY);
		}
		//画右边的
		if( right > 0 )
		{
			rcDest.SetRect(rcMem.right - right, rcMem.top + top, rcMem.right, rcMem.bottom - bottom);
			::StretchBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), hDC,
				xSrc + nSrcWidth - right, ySrc + top, right, nSrcHeight - top - bottom, SRCCOPY);
		}

		//画下边的
		if( bottom > 0 )
		{
			rcDest.SetRect(rcMem.left + left, rcMem.bottom - bottom, rcMem.right - right, rcMem.bottom);
			::StretchBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), hDC,
				xSrc + left, nSrcHeight - bottom, nSrcWidth - left - right, bottom, SRCCOPY);
		}

		//绘左上角
		if( left > 0 && top > 0 )
		{
			rcDest.SetRect(rcMem.left, rcMem.top, rcMem.left + left, rcMem.top + top);
			BitBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), xSrc, ySrc);
		...
		if( right > 0 && top > 0 )
		{
			rcDest.SetRect(rcMem.right - right, rcMem.top, rcMem.right, rcMem.top + top);
			BitBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), 
				xSrc + nSrcWidth - right, ySrc);
		}

		//绘左下角
		if( left > 0 && bottom > 0 )
		{
			rcDest.SetRect(rcMem.left, rcMem.bottom - bottom, rcMem.left + left, rcMem.bottom);
			BitBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), xSrc, ySrc + nSrcHeight - bottom);
		}

		//绘右下角
		if( right > 0 && bottom > 0 )
		{
			rcDest.SetRect(rcMem.right - right, rcMem.bottom - bottom, rcMem.right, rcMem.bottom);
			BitBlt(memDC, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(),
				xSrc + nSrcWidth - right, ySrc + nSrcHeight - bottom);
		}

		memDC.SetStretchBltMode(nMode);

		//画双缓存
		if( uFlags & DS_TRANSPARENT )
		{
			::TransparentBlt(pDC->GetSafeHdc(), lpRect->left, lpRect->top, lpRect->right - lpRect->left, 
				lpRect->bottom - lpRect->top, memDC, 0, 0, rcMem.Width(), rcMem.Height(), crTransparent);
		}
		else
		{
			::BitBlt(pDC->GetSafeHdc(), lpRect->left, lpRect->top, lpRect->right - lpRect->left, 
				lpRect->bottom - lpRect->top, memDC, 0, 0, SRCCOPY);
		}
	}
	else
	{
		...
		{
			::TransparentBlt(pDC->GetSafeHdc(), lpRect->left, lpRect->top, lpRect->right - lpRect->left,
				lpRect->bottom - lpRect->top, hDC, xSrc, ySrc, nSrcWidth, nSrcHeight, crTransparent);
		}
		else
		{
			::BitBlt(pDC->GetSafeHdc(), lpRect->left, lpRect->top, lpRect->right - lpRect->left,
				lpRect->bottom - lpRect->top, hDC, 0, 0, SRCCOPY);
		}
	}
...
bool CSkinImage::DrawSkin( CDC* pDC, LPRECT lpRect, UINT uFlags, int left, int top, int right, int bottom, COLORREF crTransparent/*=RGB(255,0,255)*/ )
{
	return DrawSkin(pDC, lpRect, 0, 0, GetWidth(), GetHeight(), uFlags, left, top, right, bottom, crTransparent);
}
/////////////////////////////////////////////////////////////////////////////////////////
