...
VOID CSkinButton::DrawButtonText(CDC * pDC, UINT uItemState)
{
	...
	if (strText.IsEmpty()==FALSE)
	{
		...
		CFont *pOldTextFont=NULL;
		if(m_ptextFont != NULL)
			pOldTextFont = (CFont *)::SelectObject(pDC->m_hDC, *m_ptextFont);
		pDC->DrawText(strText,strText.GetLength(),rcClient,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		if (m_ptextFont != NULL)
			::SelectObject(pDC->m_hDC, pOldTextFont);
	}
...
//设置字体与颜色
void CSkinButton::SetTextFont(CFont *ptextFont, COLORREF crText)
{
	m_ptextFont = ptextFont;
	m_crButtonText = crText;
}
