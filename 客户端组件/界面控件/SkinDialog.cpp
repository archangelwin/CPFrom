...
CSkinDialogAttribute::CSkinDialogAttribute()
{
	...
	m_crBackGround=RGB(222,239,246);
...
//设置区域
VOID CSkinLayered::InitLayeredArea(CPngImageSB & Image, BYTE cbAlpha, CRect & rcUnLayered, CPoint & PointRound, bool bUnLayeredChild)
{
	//创建缓冲
	CImage ImageBuffer;
...
VOID CSkinDialog::DrawControlView(CDC * pDC, INT nWdith, INT nHeight)
{
	...
	rcClientView.left=/*m_SkinAttribute.m_EncircleInfoFrame.nLBorder*/0;
...
VOID CSkinDialog::OnWindowPosChanged(WINDOWPOS * lpWndPos)
{
	...
	//CWnd *pOldWind = GetFocus();
	//移动分层
	//if ((m_SkinLayered.m_hWnd!=NULL)&&(lpWndPos->cx>=0)&&(lpWndPos->cy>0))
	//{
	//	m_SkinLayered.SetWindowPos(NULL,lpWndPos->x,lpWndPos->y,lpWndPos->cx,lpWndPos->cy,SWP_NOZORDER|SWP_NOACTIVATE);
	//}
	/*if(pOldWind != NULL)
		pOldWind->SetFocus();*/
...
VOID CALLBACK CSkinDialog::SetWndFont(CWnd *pWnd, CFont *pFont)
{
	...
	ChildParam.pWndControl = pWnd;
	ChildParam.pFont = pSetFont;
	EnumChildWindows(pWnd->m_hWnd,CSkinDialog::EnumChildProc,(LPARAM)&ChildParam);
}
//////////////////////////////////////////////////////////////////////////////////
