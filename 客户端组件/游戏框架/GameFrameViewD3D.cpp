...
VOID CGameFrameViewD3D::DrawMatchInfo(CD3DDevice * pD3DDevice)
{
	...
	if(m_pMatchInfo!=NULL)
	{
		...
		ImageMatchInfo.DrawImage(pD3DDevice,10,10,(BYTE)210);
		
		CString strNum;
		strNum.Format(TEXT("%d"),m_pMatchInfo->wGameCount);
		DrawTextString(pD3DDevice,strNum,12+10,240+10,DT_LEFT,D3DCOLOR_XRGB(255,255,255));
		
		DrawTextString(pD3DDevice,m_pMatchInfo->szTitle[0],38+10,10+10,DT_LEFT,D3DCOLOR_XRGB(196,221,239));
		DrawTextString(pD3DDevice,m_pMatchInfo->szTitle[1],38+10,30+10,DT_LEFT,D3DCOLOR_XRGB(196,221,239));
		DrawTextString(pD3DDevice,m_pMatchInfo->szTitle[2],38+10,50+10,DT_LEFT,D3DCOLOR_XRGB(196,221,239));
		DrawTextString(pD3DDevice,m_pMatchInfo->szTitle[3],38+10,70+10,DT_LEFT,D3DCOLOR_XRGB(255,221,35));
	}
	...
	if(m_pMatchWaitTip!=NULL)
	{
		...
		DrawNumberString(pD3DDevice, ImageNumberOrange, (LONG)m_pMatchWaitTip->lScore, nXPos+118+20, nYPos+68);
		...
		strDrawText.Format(TEXT("您已经率先完成本轮，处于本桌第%d名。请等待其他桌游戏结束"),m_pMatchWaitTip->wCurTableRank);
		...
		m_D3DFont.DrawText(pD3DDevice,TEXT("后确定最后排名。"), &CRect(nXPos+18,nYPos+168, nXPos+ImageWaitTip.GetWidth()-36, nYPos+200),
			DT_TOP|DT_WORDBREAK,D3DCOLOR_XRGB(200,246,244));
