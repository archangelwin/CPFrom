...
/*	if ((bMeUserItem==false)&&(cbMeUserStatus!=US_PLAYING)&&(cbUserStatus==US_LOOKON))
	...
	FLOAT fUserFleeRate=pIClientUserItem->GetUserFleeRate();
// 	_sntprintf(szMenuString,CountArray(szMenuString),TEXT("金币：%I64d  经验值：%ld 总局数：%ld  逃跑率：%5.2f%%"),
// 		pUserInfo->lScore,pUserInfo->dwExperience,dwPlayCount,fUserFleeRate);
// 	_sntprintf(szMenuString,CountArray(szMenuString),TEXT("金币：%I64d  经验值：%ld  逃跑率：%5.2f%%"),
// 		pUserInfo->lScore,pUserInfo->dwExperience,fUserFleeRate);
	//插入分割
	UserInfoMenu.AppendSeparator();
	...
	{
		_sntprintf(szMenuString,CountArray(szMenuString),TEXT("金币：%I64d  等级：%d 级"),
		pUserInfo->lScore,pUserInfo->dwExperience);
	}else
		_sntprintf(szMenuString,CountArray(szMenuString),TEXT("金币：%I64d"),
		pUserInfo->lScore);
...
VOID CGameFrameControl::OnEventUserLeave(IClientUserItem * pIClientUserItem, bool bLookonUser)
{
	...
	//ccy20141205
	tagServerAttribute * pServerAttribute=m_pIClientKernel->GetServerAttribute();
	if((pServerAttribute->wServerType&GAME_GENRE_MATCH)==0)
	{
		//删除用户
...
// 		m_GameFrameUserView.ShowWindow(SW_HIDE);
// 	}
	...
	case IDC_BT_CHAT_SHORT:			//聊天短语
		{
			...
			_sntprintf(szFileName2,CountArray(szFileName2),TEXT("%s\\LobbyDate\\PhraseInfo.ini"),szDirectory);
...
VOID CGameFrameControl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	rcListArea.top=USER_VIEW_CY/*-80*/;
	rcListArea.bottom=USER_VIEW_CY+(nHeight-USER_VIEW_CY-MEDAL_VIEW_CY)*INCISE_SCALE-PROPERTY_CY+65;
	...
	//聊天控件
	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatArea.right-34,rcChatArea.bottom-24,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChatShort,NULL,rcChatArea.right-52,rcChatArea.bottom-24,0,0,uFlags|SWP_NOSIZE);
	...
	//其他控件
	DeferWindowPos(hDwp,m_GameFrameUserView,NULL,0,0,nWidth,USER_VIEW_CY/*-80*/,uFlags);
...
//	DeferWindowPos(hDwp,m_GamePropertyCtrl,NULL,0,rcListArea.bottom+MEDAL_VIEW_CY,nWidth,PROPERTY_CY,uFlags);
	...
	if ((m_ChatControl.m_hWnd!=NULL)&&(m_ChatControl.GetChatTargetUserItem()!=NULL))
	{
		...
		rcChatArea.bottom-=rcChatControl.Height();
 		CRect rtTemp = rcChatArea;
 		rtTemp.top -= 60;
		m_ChatEncircle.DeferWindowPos(&m_ChatMessage,hDwp,rtTemp);
...
BOOL CGameFrameControl::OnEraseBkgnd(CDC * pDC)
{
	...
	rcChatArea.top=USER_VIEW_CY+MEDAL_VIEW_CY+(rcClient.Height()-USER_VIEW_CY-MEDAL_VIEW_CY)*INCISE_SCALE-60;
	...
	rcUserArea.bottom=USER_VIEW_CY+(rcClient.Height()-USER_VIEW_CY-MEDAL_VIEW_CY)*INCISE_SCALE-PROPERTY_CY+65;
