...
VOID CGameFrameViewGDI::DrawUserClock(CDC * pDC, INT nXPos, INT nYPos, WORD wUserClock)
{
	...
	if ((wUserClock>0)&&(wUserClock<100))
	{
		...
		if (RealizeWIN7() && CD2DEngine::GetD2DEngine())
		{
			...
			m_ImageClockItemEx.DrawImage(pDC,nXDrawPos+13+9,nYDrawPos+27,SizeClockItem.cx,SizeClockItem.cy,nClockItem1*SizeClockItem.cx,0);
			m_ImageClockItemEx.DrawImage(pDC,nXDrawPos+28+9,nYDrawPos+27,SizeClockItem.cx,SizeClockItem.cy,nClockItem2*SizeClockItem.cx,0);
		}
		else
		{
			//加载资源
			CPngImageSB ImageClockItem;
			CPngImageSB ImageClockBack;
			...
			WORD nClockItem1=wUserClock/10;
			WORD nClockItem2=wUserClock%10;
			ImageClockItem.DrawImage(pDC,nXDrawPos+13+5,nYDrawPos+27-8,SizeClockItem.cx,SizeClockItem.cy,nClockItem1*SizeClockItem.cx,0);
			ImageClockItem.DrawImage(pDC,nXDrawPos+28+5,nYDrawPos+27-8,SizeClockItem.cx,SizeClockItem.cy,nClockItem2*SizeClockItem.cx,0);
		}
	}
...
//绘画数字
VOID CGameFrameViewGDI::DrawNumberString(CDC * pDC, CPngImageSB &PngNumber, LONG lNumber, INT nXPos, INT nYPos,BYTE cbAlign)
{
	//加载资源
...
VOID CGameFrameViewGDI::DrawMatchInfo(CDC * pDC)
{
	...
	{
		CPngImageSB ImageWaitDistribute;
	...
	{
		CPngImageSB ImageMatchInfo;
		ImageMatchInfo.LoadImage(GetModuleHandle(GAME_FRAME_DLL_NAME),TEXT("MATCH_INFO"));
		ImageMatchInfo.AlphaDrawImage(pDC,10,10,210);
		pDC->SetTextColor(RGB(255,255,255));
		CString strNum;
		strNum.Format(TEXT("%d"),m_pMatchInfo->wGameCount);
		pDC->TextOut(12+10,40+10,strNum); 
		pDC->SetTextColor(RGB(196,221,239));
		pDC->TextOut(38+10,10+10,m_pMatchInfo->szTitle[0]);
		pDC->TextOut(38+10,30+10,m_pMatchInfo->szTitle[1]);
		pDC->TextOut(38+10,50+10,m_pMatchInfo->szTitle[2]);
		pDC->SetTextColor(RGB(255,221,35));
		pDC->TextOut(38+10,70+10,m_pMatchInfo->szTitle[3]);
	}
	...
	{
		//定义变量
		CPngImageSB ImageWaitTip;
		CPngImageSB ImageNumberOrange;
		CPngImageSB ImageNumberGreen;
		CPngImageSB ImageLine;
		...
		INT nYPos=(rcClient.Height()-ImageWaitTip.GetHeight())/2/*-40*/;
		...
		DrawNumberString(pDC, ImageNumberOrange, (LONG)m_pMatchWaitTip->lScore, nXPos+158, nYPos+70-7);
		DrawNumberString(pDC, ImageNumberOrange, m_pMatchWaitTip->wRank, nXPos+250+20,nYPos+70-8); 
		ImageLine.DrawImage(pDC, nXPos+265+20, nYPos+60-8);
		DrawNumberString(pDC, ImageNumberOrange, m_pMatchWaitTip->wUserCount, nXPos+280+20,nYPos+70-7,TA_RIGHT);
		...
		DrawNumberString(pDC, ImageNumberGreen, m_pMatchWaitTip->wPlayingTable, nXPos+160,nYPos+110+7,TA_RIGHT);
		...
		DrawNumberString(pDC, ImageNumberOrange, m_pMatchWaitTip->wCurTableRank, nXPos+160+77+5,nYPos+110+10+40+5,TA_RIGHT);
