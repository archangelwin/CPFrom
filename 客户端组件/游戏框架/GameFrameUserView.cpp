...
//控件标示
#define IDC_WEB_PUBLICIZE			0/*100*/									//浏览控件
...
// 		_sntprintf(szString,CountArray(szString),TEXT("金币：") SCORE_STRING TEXT(" 等级：%s [ %s ]"),pUserInfo->lScore,pszGameLevel,pGameAttribute->szGameName);
// 		pBufferDC->DrawText(szString,lstrlen(szString),&rcGameLevel,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		LPCTSTR pszGameLevel=pIGameLevelParser->GetLevelDescribe(m_pICurrentUserItem);
		_sntprintf(szString,CountArray(szString),TEXT("金币：") SCORE_STRING,pUserInfo->lScore);
		pBufferDC->DrawText(szString,lstrlen(szString),&rcGameLevel,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);

		CGlobalServer * pGlobalServer = CGlobalServer::GetInstance();
		if (pGlobalServer->BShowGameLevel() == true)
		{
			_sntprintf(szString,CountArray(szString),TEXT("等级：%d级 [ %s ]"),dwExperience,pszGameLevel/*,pGameAttribute->szGameName*/);
		}else
			_sntprintf(szString,CountArray(szString),TEXT("等级：%s [ %s ]"),pszGameLevel,pGameAttribute->szGameName);//		LPCTSTR pszGameLevel=pIGameLevelParser->GetLevelDescribe(m_pICurrentUserItem);
//		_sntprintf(szString,CountArray(szString),TEXT("等级：%d级 [ %s ]"),dwExperience,pszGameLevel/*,pGameAttribute->szGameName*/);
...
// 		_sntprintf(szString,CountArray(szString),TEXT("胜率：%.2f%% 逃率：%.2f%%"),fUserWinRate,fUserFleeRate);
// 		pBufferDC->DrawText(szString,lstrlen(szString),&rcGameDrawInfo,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
	}
