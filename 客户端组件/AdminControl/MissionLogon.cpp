...
// int g_nProgress = 0;
// ProgressWndDlg *pProDlg;
//////////////////////////////////////////////////////////////////////////////////
CWinThread* ConnectThread=NULL;
...
//	ASSERT(pMissionLogon->GetMissionManager()!=NULL);
	while (m_IsThreadRun)
	{
		if (pMissionLogon == NULL && pMissionUserInfo == NULL)
		{
// 			/*CMissionLogon* */pMissionLogon = CMissionLogon::GetInstance();
// 			/*CMissionUserInfo**/ pMissionUserInfo = CMissionUserInfo::GetInstance();
			m_IsThreadRun = false;
		...
		bool n = pMissionLogon->GetMissionManager()->SendData(MDM_UM_USER,SUB_UM_HEART_BEAT);
		if (n == false)
		{
			pMissionLogon->PerformLogonMission(true);
			pMissionUserInfo->m_GameUserListDlg->OnIsConnect(false);
			Sleep(30000);
		}else
		{
			Sleep(10000);
			pMissionUserInfo->m_GameUserListDlg->OnIsConnect(true);
		}
...
BEGIN_MESSAGE_MAP(CMissionLogon, CDialog)
END_MESSAGE_MAP()
...
// 	m_IsThreadRun = false;
// 	ConnectThread->ResumeThread();
// 	::WaitForSingleObject(ConnectThread->m_hThread,INFINITE);
...
// 		AfxMessageBox("尝试了所有的服务器都无法成功连接服务器");
// 
// 		//重新登录
// // 		if (m_bRegister==false)
// // 		{
...
// 	if (GetActiveStatus()==true) return false;
	...
	{
// 		if (pMissionManager->m_MissionSocket.m_TCPSocketModule->GetSocketStatus() == SOCKET_STATUS_WAIT)
// 		{
// 
// 			return false;
// 		}
		...
		AfxMessageBox("服务器连接失败，请检查网络配置以及网络连接状况！");
		...
		pServerUserInfoData->m_pIServerUserInfoDataSink->OnDelAllUserData();
...
// 	if (pMissionManager!=NULL) pMissionManager->ConcludeMissionItem(this,false);
	...
	//显示消息
 	...
 	if (lstrlen(pszDescribeString)>0) //ShowInformation(pszDescribeString,MB_ICONERROR,60);
		MessageBox(pszDescribeString,NULL,NULL);
...
// 	if (pMissionManager!=NULL) pMissionManager->ConcludeMissionItem(this,false);
	...
	if (pPlatformEvent!=NULL) pPlatformEvent->SendPlatformEvent(EVENT_ADMIN_LOGON,0L);
	...
	CUserRemarks::GetInstance()->CreatrUserRemarksTable();
	
	CMissionUserInfo* pMissionUserInfo = CMissionUserInfo::GetInstance();
	CMissionUserInfo::GetInstance()->m_GameUserListDlg->OnIsConnect(true);
	//网络加载
