...
// 	}
	...
	//更新人数
	if (/*m_bUpdateOnLine==true&&*/m_IsFirstLog==true)
	{
		//设置变量
// 		m_bUpdateOnLine=false;
// 
...
// 				m_tabServerID.dwServerID = GetOnline.wOnLineServerID[GetOnline.wServerCount];
// 			}
// 
// 		} while (Position!=NULL);
// 
// 		CGameServerItem * pGameServerItem=pServerListData->EmunGameServerItem(Position);
// 
// 		m_tabServerID.dwServerID = pGameServerItem->m_GameServer.wServerID;
// 
// 		//发送数据
...
//		pMissionManager->SendData(MDM_GP_SERVER_LIST,SUB_GP_GET_ONLINE,&GetOnline,wSendSize);
		pMissionManager->SendData(MDM_UM_USER,SUB_UM_GET_USER_GAME_SET,&m_ServerID,sizeof(m_ServerID));
		
		pMissionManager->SendData(MDM_UM_USER,SUB_UM_GET_SYS_SET,&m_ServerID,sizeof(m_ServerID));
	}
...
// 		//发送数据
// 		CMissionManager * pMissionManager=GetMissionManager();
// 		pMissionManager->SendData(MDM_UM_USER,SUB_GP_GET_SERVER,wKindIDItem,wKindIDCount*sizeof(WORD));
// 	}
...
// 				CUserServerInfo * pServerUserInfoData=CUserServerInfo::GetInstance();
// 
// 				pServerUserInfoData->m_szGameName = pGameServer->szServerName;
