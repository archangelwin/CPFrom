	...
	//绘画子项
 	do
 	{
 		...
 		if (GetItemRect(hItemCurrent,rcItem,TRUE))
 		{
 			...
 			{
 				pDC->FillSolidRect(&rcRect,/*RGB(255,255,255)*/pSkinRenderManager->RenderColor(0.36,1.00));
 			}
...
{
	//更新人数
	CMyFrameWnd::GetInstance()->UpDataAllOnLineCount();
...
VOID CServerListView::OnNMLClick(NMHDR * pNMHDR, LRESULT * pResult)
{
	...
	{
		if (pServerUserInfoData->OnInsertGameUserInfo((CGameServerItem *)pGameListItem) == false)
		{
			//AfxMessageBox("当前没有游戏!");
		}
		ASSERT(CMissionUserInfo::GetInstance()!=NULL);
// 		tagUMUserGameSet* ptagUMUserGameSet = new tagUMUserGameSet;
// 		CMissionUserInfo::GetInstance()->m_GameUserListDlg->GetSysUserDataInfo(ptagUMUserGameSet);
	...
	if ((pGameListItem!=NULL)&&(pGameListItem->GetItemGenre()==ItemGenre_Kind))
	{
		if (pServerUserInfoData->OnInsertGameKindInfo((CGameKindItem *)pGameListItem) == false)
		{
			//AfxMessageBox("当前没有房间!");
		}
...
//		pPlatformFrame->EntranceServerItem((CGameServerItem *)pGameListItem);
		...
		if (pServerUserInfoData->OnUpdataGameUserInfo((CGameServerItem *)pGameListItem) == false)
		{
			//AfxMessageBox("当前没有房间!");
		}
		...
		//225
		ASSERT(CMissionList::GetInstance()!=NULL);
		CMissionList::GetInstance()->m_IsFirstLog=true;
		CMissionList::GetInstance()->m_ServerID = ((CGameServerItem *)pGameListItem)->m_GameServer.wServerID;
		CMissionList::GetInstance()->UpdateOnLineInfo();
