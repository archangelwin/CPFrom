...
bool CDownLoad::OnSocketLink(INT nErrorCode)
{
	...
	if (nErrorCode!=0)
	{
		...
		if (m_pIDownLoadSink!=NULL) 
			m_pIDownLoadSink->OnDownLoadError(DownLoadError_Connect,nErrorCode,"Link");
...
bool CDownLoad::OnSocketShut(BYTE cbShutReason)
{
	if (cbShutReason!=SHUT_REASON_NORMAL)
	{
		...
		ASSERT(m_pIDownLoadSink!=NULL);
		if (m_pIDownLoadSink!=NULL)
			m_pIDownLoadSink->OnDownLoadError(DownLoadError_Server,cbShutReason,"Shut");
	}
...
VOID CDownLoad::OnSocketDataMain(VOID * pcbMailData, WORD wStreamSize)
{
	...
	ASSERT(m_pIDownLoadSink!=NULL);
	if (m_pIDownLoadSink!=NULL) 
		m_pIDownLoadSink->OnDataStream(pcbMailData,wStreamSize);
	...
	case HTTP_STATUS_OK:				//获取成功
		{
			...
			ASSERT(m_pIDownLoadSink!=NULL);
			if (m_pIDownLoadSink!=NULL) 
				m_pIDownLoadSink->OnDownLoadStatus(DownLoadStatus_Service);
	...
	default:							//默认处理
		{
			...
			ASSERT(m_pIDownLoadSink!=NULL);
			if (m_pIDownLoadSink!=NULL) 
				m_pIDownLoadSink->OnDownLoadError(DownLoadError_Server,nStatusCode,"DataHead");
...
bool CDownLoad::ConnectServer(LPCTSTR pszUrl)
{
	...
	if (cbResult==CONNECT_SUCCESS)
	{
		...
		ASSERT(m_pIDownLoadSink!=NULL);
		if (m_pIDownLoadSink!=NULL) 
			m_pIDownLoadSink->OnDownLoadStatus(DownLoadStatus_Connect);
	}
	else
	{
		//停止下载
		...
		ASSERT(m_pIDownLoadSink!=NULL);
		if (m_pIDownLoadSink!=NULL) 
			m_pIDownLoadSink->OnDownLoadError(DownLoadError_Server,-100,"connect");
...
bool CDownLoad::SendRequestData(LPCTSTR pszHostName, LPCTSTR pszUrlPath)
{
	...
	_snprintf(szRequestData,CountArray(szRequestData),"GET /%s HTTP/1.1\r\nAccept:text/html, application/xhtml+xml, */*\r\nAccept-Language: zh-CN\r\nUser-Agent:Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; Trident/6.0)\r\n"
		"Accept-Encoding: gzip, deflate\r\nHost:%s\r\nDNT: 1\r\nConnection:Keep-Alive\r\nCookie: ASP.NET_SessionId=%s\r\n\r\n",
		(LPCSTR)strUrlPath,(LPCSTR)strHostName,(LPCSTR)chRand);
...
INTERNET_SCHEME CDownLoad::GetDownLoadInfo(LPCTSTR pszUrl, tagDownLoadInfo & DownLoadInfo)
{
	...
	if (InternetCanonicalizeUrl(pszUrl,szDownLoadUrl,&dwMaxLength,dwFlags)==FALSE) 
	...
	if (InternetCrackUrl(szDownLoadUrl,CountArray(szDownLoadUrl),ICU_ESCAPE,&Components)==FALSE) 
		return INTERNET_SCHEME_UNKNOWN;
