...
CWEBSocket::CWEBSocket()
{
	...
	//代理信息
	m_cbProxyType=enProxyServerType::ProxyType_None;
...
bool CWEBSocket::SetProxyServerInfo(BYTE cbProxyType, const tagProxyServer & ProxyServer)
{
	switch (cbProxyType)
	{
	case enProxyServerType::ProxyType_None:	//没有代理
		{
			//设置变量
	...
	case enProxyServerType::ProxyType_Http:	//HTTP 代理
	case enProxyServerType::ProxyType_Socks4:	//SOKKS4 代理
	case enProxyServerType::ProxyType_Socks5:	//SOKKS5 代理
		{
			//设置变量
			m_cbProxyType=cbProxyType;
...
VOID CWEBSocket::CloseSocket(BYTE cbShutReason)
{
	...
	if (m_hSocket!=INVALID_SOCKET)
	{
		...
		if (cbShutReason!=SHUT_REASON_INSIDE)
		{
			...
			if (m_pIWEBSocketSink!=NULL) 
				m_pIWEBSocketSink->OnSocketShut(cbShutReason);
		}
	}
...
BYTE CWEBSocket::ConnectServer(LPCTSTR pszServer, WORD wPort)
{
	...
	try
	{
		...
		//代理连接
		if (m_cbProxyType!=enProxyServerType::ProxyType_None)
		{
			//连接代理
			...
			case enProxyServerType::ProxyType_Socks5:	//Socks5 代理
				{
					//构造请求
					INT nOff=4;
			...
			case enProxyServerType::ProxyType_Socks4:	//Socks4 代理
				{
					//构造请求
					INT nOff=2;
		...
		else
		{
			...
			if ((nErrorCode==SOCKET_ERROR)&&(nWsaLastError!=WSAEWOULDBLOCK)) 
...
{
	//效验状态
	ASSERT(m_cbProxyType!=enProxyServerType::ProxyType_None);
	if (m_cbProxyType==enProxyServerType::ProxyType_None) return CONNECT_EXCEPTION;
	...
	//发送数据
	if (m_cbProxyType==enProxyServerType::ProxyType_Socks5)
	{
		//变量定义
		INT nOff=2;
...
LRESULT CWEBSocket::OnSocketNotifyRead(WPARAM wParam, LPARAM lParam)
{
	try
	{
		...
		/*	if (nResult == 330)
			{
				nResult=recv(m_hSocket,m_szHeadData+m_wHeadSize,sizeof(m_szHeadData)-m_wHeadSize,0);
			}*/
			...
			if (pszFinality!=NULL)
			{
				...
				for (WORD i=0;i<m_wHeadSize;i++)
				{
					if (m_szHeadData[i]==' ')
					{
						...
						do
						{
							if ((nStatusOff<(CountArray(szStatusString)-1))&&(i<m_wHeadSize-1))
							{
