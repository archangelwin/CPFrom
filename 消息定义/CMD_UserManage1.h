...
struct tagUMUserScoreSet
{
	...
	SCORE						lMaxWin1;							//赢分数(保留)---(当天输赢金币)
	SCORE						lMaxLose1;							//输分数(保留)---（充值）
	...
	DWORD						dwWinOrLostRate1;					//换牌比率（百分比）（保留)(渠道：1：试玩站；0：正常)
	int							nVipType;							//用户类型 （如vip） （1：vip 0：非vip 3：代理）
...
struct tagUMSysGameSet
{
 ...
 //	TCHAR szItem0[64];									//[ %s ]上庄了，让我们给他点颜色看看
	...
	SCORE AllScore;										//所有累计
	SCORE MonthScore;									//当月累计
	SCORE WeekScore;									//本周累计
