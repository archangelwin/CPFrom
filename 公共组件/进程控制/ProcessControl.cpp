...
//创建通讯
bool CProcessControl::SetProcessControlSinks(IUnknownEx * pIUnknownEx)
{
	//设置接口
	if (pIUnknownEx!=NULL)
	{
	...
	case IDT_IMMEDIATE:			//同步数据
		{
			...
			m_pIProcessControlSink->OnProcessControlDatas(pHead->wMainCmdID,pHead->wSubCmdID,pHead+1,wDataSize,pWnd->m_hWnd);
...
LRESULT	CProcessControl::OnMessageAsynchronismData(WPARAM wParam, LPARAM lParam)
{
	...
	m_pIProcessControlSink->OnProcessControlDatas(wMainCmdID,wSubCmdID,AsynchronismData.cbBuffer,AsynchronismData.wDataSize,hWndClient);
