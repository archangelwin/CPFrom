...
//设置接口
bool CProcessRecvData::SetProcessTradeSinks(IUnknownEx * pIUnknownEx,bool bSet)
{
	//设置接口
	if (pIUnknownEx!=NULL)
	{
	...
	case IDT_ASYNCHRONISM:		//异步数据
		{
			//数据处理
			WORD wDataSize=pHead->wPacketSize-sizeof(IPC_Head);
			m_pIIPCRecvDataSink->OnAsynchronismmData(pHead->wMainCmdID,pHead->wSubCmdID,pHead+1,wDataSize,hWndSend);
