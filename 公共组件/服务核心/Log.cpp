...
void CLog::Write(BYTE *pData, int nLen)
{
	ASSERT(pData != NULL);
...
bool CLog::Init(LPCTSTR sFileName)
{
	m_nType = 0;
...
void CLog::WriteData(LPCTSTR szData)
{
	m_crtFile.Lock();
	theLog.Write(szData);
}

void CLog::WriteMsg(LPCTSTR sData)
{
	if (!m_bOpen)
		return;
	m_crtFile.Lock();
...
void CLog::ClearFile(LPCTSTR sFileName)
{
	m_crtFile.Lock();
	m_file.SetLength(0);
	m_crtFile.Unlock();
...
void CLog::CloseFile(LPCTSTR sFileName)
{
	m_crtFile.Lock();
	m_file.Close();
	m_bOpen = false;
